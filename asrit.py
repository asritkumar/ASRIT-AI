    ...
    try:
        async for raw in google_ws:
            try:
                ...
                continue
            # Preserve the provider message exactly so the browser can render
            # setup errors, audio, transcripts, turnComplete, goAway, etc.
            await browser_ws.send_json(data)
    except Exception as error:
        try:
            await browser_ws.send_json({
                "type": "error",
                "message": f"Gemini Live transport error: {error}"
            })
        except Exception:
            pass
    else:
        # A clean provider close should never look like a mysterious
        # "Live disconnected" message in the UI.
        try:
            await browser_ws.send_json({
...
/* Legacy performance-mode comment retained; final Liquid Glass stylesheet overrides its visual rules. */
*, *::before, *::after {
...
<style id="asrit-liquid-glass-reference-v4">
/* =========================================================
   ASRIT — LIQUID GLASS REFERENCE UI v4
   Visual target: the user's white / clear-glass reference
   images. This intentionally overrides the older peach/dark
   surfaces while leaving the application logic untouched.
========================================================= */
...
:root{
  ...
  --lg-muted:#6f6f6f;
  --lg-soft:#8b8b8b;
  --lg-line:rgba(20,20,20,.12);
  --lg-line-strong:rgba(20,20,20,.18);
  --lg-glass:rgba(255,255,255,.54);
  --lg-glass-strong:rgba(255,255,255,.76);
  --lg-shadow:0 22px 60px rgba(38,40,45,.10), inset 0 1px 0 rgba(255,255,255,.92);
  --lg-spring:cubic-bezier(.16,1,.3,1);
  --lg-fluid:cubic-bezier(.22,.75,.25,1);
}

html,body{
  background:
    radial-gradient(circle at 50% 44%, rgba(255,255,255,.96) 0%, rgba(250,250,248,.94) 44%, rgba(242,243,240,.98) 100%) !important;
  color:var(--lg-ink) !important;
}

/* White studio surface + extremely subtle moving refractions. */
body{
...
body::before{
  ...
  background:
    radial-gradient(circle at 16% 18%, rgba(255,255,255,.98) 0 7%, transparent 25%),
    radial-gradient(circle at 83% 22%, rgba(255,255,255,.82) 0 5%, transparent 23%),
    radial-gradient(circle at 70% 86%, rgba(255,255,255,.86) 0 8%, transparent 25%),
    radial-gradient(circle at 28% 82%, rgba(224,229,226,.42) 0 5%, transparent 22%) !important;
  filter:blur(28px) !important;
  animation:lgAtmosphere 18s var(--lg-fluid) infinite alternate !important;
  opacity:1 !important;
}
body::after{
  content:"" !important;
  ...
  pointer-events:none !important;
  background:
    radial-gradient(circle at 12% 35%, rgba(255,255,255,.96) 0 2px, transparent 3px),
    radial-gradient(circle at 22% 16%, rgba(255,255,255,.90) 0 4px, transparent 5px),
    radial-gradient(circle at 38% 78%, rgba(255,255,255,.95) 0 3px, transparent 4px),
    radial-gradient(circle at 61% 16%, rgba(255,255,255,.92) 0 3px, transparent 4px),
    radial-gradient(circle at 78% 38%, rgba(255,255,255,.92) 0 5px, transparent 6px),
    radial-gradient(circle at 91% 73%, rgba(255,255,255,.95) 0 3px, transparent 4px),
    radial-gradient(circle at 54% 91%, rgba(255,255,255,.9) 0 4px, transparent 5px);
  filter:blur(.1px);
  animation:lgBubbles 14s ease-in-out infinite alternate !important;
...
.sidebar{
  ...
  border-right:1px solid rgba(30,30,30,.11) !important;
  border-radius:38px !important;
  background:
    linear-gradient(180deg, rgba(255,255,255,.68), rgba(250,251,249,.42)) !important;
  box-shadow:
    0 28px 80px rgba(39,42,48,.10),
    inset 0 1px 0 rgba(255,255,255,.98),
    inset 0 -1px 0 rgba(0,0,0,.035) !important;
  backdrop-filter:blur(34px) saturate(120%) !important;
  -webkit-backdrop-filter:blur(34px) saturate(120%) !important;
  color:var(--lg-ink) !important;
  overflow:hidden !important;
  animation:lgSidebarIn .8s var(--lg-spring) both !important;
}
...
.sidebar::before{
  ...
  pointer-events:none !important;
  background:linear-gradient(115deg,rgba(255,255,255,.68),transparent 28%,transparent 72%,rgba(255,255,255,.38)) !important;
  opacity:.9 !important;
}
...
.logo-orb{
  ...
  background:
    radial-gradient(circle at 34% 27%, rgba(255,255,255,.98) 0 9%, rgba(255,255,255,.46) 24%, rgba(223,228,226,.28) 53%, rgba(255,255,255,.72) 100%) !important;
  border:1px solid rgba(255,255,255,.98) !important;
  box-shadow:
    inset 0 1px 1px rgba(255,255,255,.98),
    inset 0 -8px 16px rgba(90,95,100,.08),
    0 8px 28px rgba(45,48,53,.12) !important;
  color:#171717 !important;
  font-size:30px !important;
  font-weight:400 !important;
  font-family:Georgia,"Times New Roman",serif !important;
  animation:lgOrbFloat 6s ease-in-out infinite !important;
}
.logo-orb::after{
  content:"" !important;
  position:absolute !important;
...
/* Sidebar controls become translucent pills. */
.new-chat,
.search,
.nav-item,
.history-main{
  ...
  box-shadow:
    ...
    0 6px 18px rgba(30,30,30,.035) !important;
  color:#171717 !important;
  transition:
    transform .38s var(--lg-spring),
    background .38s var(--lg-fluid),
    box-shadow .38s var(--lg-fluid),
    border-color .38s var(--lg-fluid) !important;
}
.new-chat{
...
.history-main:hover{
  ...
  box-shadow:0 12px 26px rgba(30,30,30,.07), inset 0 1px 0 #fff !important;
}
.new-chat-plus{
  width:20px !important;
...
.search::placeholder{color:#777 !important}
...
.nav-item.active{
  ...
  box-shadow:0 10px 28px rgba(30,30,30,.06), inset 0 1px 0 #fff !important;
}
.nav-icon{
  color:#202020 !important;
  opacity:.9 !important;
...
.status-dot{
  ...
  box-shadow:0 0 12px rgba(43,196,138,.28) !important;
}
.topbar > .glass-button{
  border-radius:22px !important;
  ...
  box-shadow:0 8px 25px rgba(30,30,30,.05),inset 0 1px 0 #fff !important;
}
...
.chat{
  ...
  background:
    radial-gradient(circle at 50% 38%, rgba(255,255,255,.90) 0 20%, transparent 46%),
    radial-gradient(circle at 5% 75%, rgba(255,255,255,.74) 0 7%, transparent 22%),
    radial-gradient(circle at 96% 25%, rgba(255,255,255,.74) 0 8%, transparent 24%),
    linear-gradient(180deg,#f8f9f7 0%,#f3f4f1 100%) !important;
  scrollbar-color:rgba(30,30,30,.14) transparent !important;
...
.chat::before{
  ...
  pointer-events:none !important;
  background:
    radial-gradient(ellipse 110px 55px at 7% 30%,rgba(255,255,255,.72),transparent 72%),
    radial-gradient(ellipse 140px 75px at 92% 58%,rgba(255,255,255,.62),transparent 72%),
    radial-gradient(ellipse 90px 50px at 65% 93%,rgba(255,255,255,.58),transparent 72%);
  filter:blur(3px);
  animation:lgRefraction 13s ease-in-out infinite alternate;
}
.chat::after{
  content:"" !important;
  position:absolute !important;
  ...
  pointer-events:none !important;
  background:
    radial-gradient(circle at 8% 21%, rgba(255,255,255,.95) 0 3px, rgba(180,185,184,.16) 4px, transparent 8px),
    radial-gradient(circle at 14% 75%, rgba(255,255,255,.95) 0 6px, rgba(180,185,184,.13) 7px, transparent 14px),
    radial-gradient(circle at 88% 20%, rgba(255,255,255,.95) 0 4px, rgba(180,185,184,.12) 5px, transparent 11px),
    radial-gradient(circle at 94% 72%, rgba(255,255,255,.95) 0 8px, rgba(180,185,184,.12) 9px, transparent 18px),
    radial-gradient(circle at 76% 88%, rgba(255,255,255,.95) 0 3px, rgba(180,185,184,.13) 4px, transparent 9px);
  opacity:.8;
  animation:lgDroplets 11s ease-in-out infinite alternate;
}

/* Keep actual content above decorative refractions. */
.welcome,#messages{
  position:relative !important;
  z-index:2 !important;
}

/* Central embossed glass A. */
.big-orb{
  ...
  background:
    radial-gradient(circle at 35% 25%, rgba(255,255,255,.98) 0 12%, rgba(255,255,255,.62) 30%, rgba(223,226,224,.20) 57%, rgba(255,255,255,.72) 100%) !important;
  border:1px solid rgba(255,255,255,.98) !important;
  box-shadow:
    inset 0 2px 3px rgba(255,255,255,.98),
    inset 0 -15px 24px rgba(80,85,90,.09),
    0 18px 45px rgba(50,52,55,.12),
    0 0 0 1px rgba(30,30,30,.07) !important;
  color:transparent !important;
  font-size:0 !important;
  position:relative !important;
  animation:lgHeroFloat 7s ease-in-out infinite !important;
}
.big-orb::before{
  content:"A" !important;
  position:absolute !important;
  ...
  display:grid !important;
  place-items:center !important;
  color:rgba(55,57,58,.70) !important;
  font-family:Georgia,"Times New Roman",serif !important;
  ...
  text-shadow:
    ...
    -1px -1px 1px rgba(90,90,90,.18) !important;
  -webkit-text-stroke:1px rgba(70,70,70,.12);
}
.big-orb::after{
  content:"" !important;
  position:absolute !important;
  ...
  box-shadow:inset 0 0 18px rgba(255,255,255,.62) !important;
  pointer-events:none !important;
}
.welcome{
  min-height:100% !important;
  box-sizing:border-box !important;
  padding:22px 28px 170px !important;
  display:flex !important;
  flex-direction:column !important;
  align-items:center !important;
...
.welcome-tagline span{color:#999 !important;padding:0 5px !important}

/* Two clean reference cards. */
.quick-actions{
  display:grid !important;
...
.quick-card.reference-card{
  ...
  text-align:left !important;
  background:
    linear-gradient(145deg,rgba(255,255,255,.72),rgba(255,255,255,.38)) !important;
  border:1px solid rgba(255,255,255,.96) !important;
  box-shadow:
    0 18px 38px rgba(43,45,48,.08),
    inset 0 1px 0 rgba(255,255,255,.98),
    inset 0 -1px 0 rgba(30,30,30,.035) !important;
  backdrop-filter:blur(18px) saturate(115%) !important;
  -webkit-backdrop-filter:blur(18px) saturate(115%) !important;
  transition:
    transform .48s var(--lg-spring),
    box-shadow .48s var(--lg-spring),
    background .48s var(--lg-fluid) !important;
  animation:lgCardIn .75s var(--lg-spring) both !important;
}
.reference-card:nth-child(2){animation-delay:.08s !important}
.quick-card.reference-card:hover{
  transform:translateY(-6px) scale(1.012) !important;
  background:rgba(255,255,255,.70) !important;
  box-shadow:
    0 26px 52px rgba(43,45,48,.12),
...
.reference-card-icon{
  ...
  box-shadow:inset 0 1px 0 #fff,0 6px 16px rgba(30,30,30,.05) !important;
}
...
.input-area{
  ...
  background:linear-gradient(180deg,rgba(246,247,244,0),rgba(246,247,244,.74) 34%,rgba(246,247,244,.90) 100%) !important;
  pointer-events:none !important;
}
.input-glass{
  ...
  gap:8px !important;
  background:
    linear-gradient(180deg,rgba(255,255,255,.70),rgba(255,255,255,.48)) !important;
  border:1px solid rgba(255,255,255,.98) !important;
  box-shadow:
    0 24px 55px rgba(40,42,45,.13),
    inset 0 1px 0 rgba(255,255,255,.98),
    inset 0 -1px 0 rgba(30,30,30,.04) !important;
  backdrop-filter:blur(28px) saturate(125%) !important;
  -webkit-backdrop-filter:blur(28px) saturate(125%) !important;
  transition:transform .42s var(--lg-spring),box-shadow .42s var(--lg-fluid) !important;
}
.input-glass:focus-within{
  transform:translateY(-3px) !important;
  box-shadow:0 30px 64px rgba(40,42,45,.16),inset 0 1px 0 #fff !important;
}
#messageInput{
  background:transparent !important;
  border:0 !important;
...
#messageInput::placeholder{color:#777 !important}
.input-glass .icon-button{
  ...
  border:1px solid rgba(30,30,30,.10) !important;
  background:rgba(255,255,255,.58) !important;
  box-shadow:inset 0 1px 0 #fff,0 5px 15px rgba(30,30,30,.05) !important;
  transition:transform .34s var(--lg-spring),background .34s ease,box-shadow .34s ease !important;
}
.input-glass .icon-button:hover{
  transform:scale(1.07) !important;
  background:rgba(255,255,255,.82) !important;
  box-shadow:inset 0 1px 0 #fff,0 10px 22px rgba(30,30,30,.09) !important;
}
.plus-mark,.send-mark,.mic-mark{color:#171717 !important}
.mic-mark{border-color:#171717 !important}
.mic-mark::before{background:#171717 !important}
.mic-mark::after{border-color:#171717 !important}

/* Hide old floating/mobile dock on desktop, keep it functional on mobile. */
.mobile-dock{display:none !important}

/* Message surfaces should look like soft glass, not dark chat bubbles. */
.message.user .message-bubble,
.message.assistant .message-bubble{
  background:rgba(255,255,255,.60) !important;
  color:#171717 !important;
  border:1px solid rgba(255,255,255,.92) !important;
  box-shadow:0 12px 28px rgba(30,30,30,.06),inset 0 1px 0 #fff !important;
  backdrop-filter:blur(16px) !important;
  -webkit-backdrop-filter:blur(16px) !important;
}
.message-bubble *{color:#171717 !important}

/* Image/feature panels also inherit the white glass language. */
.image-studio,.feature-screen,.history-screen{
  background:rgba(247,248,246,.94) !important;
}
.image-studio-card,.feature-shell,.history-shell{
  background:rgba(255,255,255,.64) !important;
  border-color:rgba(255,255,255,.96) !important;
  box-shadow:0 24px 60px rgba(30,30,30,.09),inset 0 1px 0 #fff !important;
  backdrop-filter:blur(24px) !important;
  -webkit-backdrop-filter:blur(24px) !important;
}

/* ---------- Subtle liquid motion ---------- */
@keyframes lgAtmosphere{
  0%{transform:translate3d(-1.5%,0,0) scale(1)}
  100%{transform:translate3d(1.5%,-1%,0) scale(1.035)}
}
@keyframes lgBubbles{
  0%{transform:translate3d(0,0,0) scale(1)}
  100%{transform:translate3d(-6px,-10px,0) scale(1.015)}
}
@keyframes lgRefraction{
  0%{transform:translate3d(-1%,0,0) scale(1)}
  100%{transform:translate3d(1%,1%,0) scale(1.035)}
}
@keyframes lgDroplets{
  0%{transform:translate3d(0,0,0)}
  100%{transform:translate3d(5px,-9px,0)}
}
...
/* Mobile: same white glass identity, reflowed like a premium app. */
@media (max-width:800px){
  .app{padding:10px !important;gap:0 !important}
  .main{border-radius:28px !important}
  .sidebar{
    position:fixed !important;
    left:10px !important;
    top:10px !important;
  ...
  .topbar .icon-button[title="Menu"]{display:grid !important}
  .topbar .glass-button{min-height:40px !important}
  .welcome{padding:18px 16px 150px !important}
  .big-orb{width:112px !important;height:112px !important;margin:18px auto 20px !important}
  .big-orb::before{font-size:68px !important}
  .welcome h1{font-size:54px !important}
  .welcome-tagline{font-size:16px !important;margin:10px 0 30px !important}
  .quick-actions{grid-template-columns:1fr !important;width:min(100%,360px) !important;gap:12px !important}
  .quick-card.reference-card{min-height:96px !important}
  .input-area{padding:12px 10px 16px !important}
  .input-glass{width:100% !important;min-height:60px !important;border-radius:30px !important}
}
@media (prefers-reduced-motion:reduce){
  *,*::before,*::after{
        ...
        >
            ...
            >
                ...
                <p class="welcome-tagline">
                    Personal AI <span aria-hidden="true">•</span> Liquid Glass Edition
                </p>

                <div class="quick-actions">
                    <button class="quick-card reference-card" onclick="openCamera()">
                        <span class="reference-card-icon" aria-hidden="true">⌁</span>
                        <span>
                            <strong>Voice Mode</strong>
                            <small>Real-time conversational speech</small>
                        </span>
                    </button>
                    <button class="quick-card reference-card" onclick="openHistoryScreen()">
                        <span class="reference-card-icon memory-icon" aria-hidden="true">◌</span>
                        <span>
                            <strong>Memory &amp; Context</strong>
                            <small>Persistent recall &amp; personalization</small>
                        </span>
                    </button>
                </div>
...
function handleLiveServerMessage(data) {
    if (data.type === "error") {
        liveSetState(data.message || "Live error", false);
        console.error("ASRIT Live provider error:", data);
        return;
    }

    if (data.type === "provider_closed") {
        liveSetState(data.message || "Gemini Live closed the session", false);
        console.warn("ASRIT Live provider closed:", data);
        return;
    }

    if (data.goAway) {
        liveSetState("Gemini Live is reconnecting…", true);
        return;
    }
...
async function openCamera() {
    ...
    try {
        ...
        liveSetState("Live could not connect: " + message, false);
        showPasteHint("Live error: " + message);
        // Keep the actual camera error visible in the Live sheet for debugging
        // instead of hiding it behind a browser alert.
        setTimeout(() => {
            closeCamera();
        }, 3200);
    }
...
async function rebindLiveAudioToStream(stream) {
    if (!liveAudioContext || !stream) return;

    if (liveProcessor) { try { liveProcessor.disconnect(); } catch (_) {} liveProcessor = null; }
    ...
    liveProcessor.onaudioprocess = function(event) {
        ...
        if (!liveSocket || liveSocket.readyState !== WebSocket.OPEN) return;
        const input = event.inputBuffer.getChannelData(0);
        const pcmBytes = floatTo16BitPCM(input, liveAudioContext.sampleRate, 16000);
        liveSocket.send(JSON.stringify({ type: "audio", data: base64FromBytes(pcmBytes) }));
    };
...
async function acquireCameraForFacing(facing) {
    const audioTrack = cameraStream?.getAudioTracks?.()[0] || null;
    const videoTrack = cameraStream?.getVideoTracks?.()[0] || null;

    // First try the existing camera track. This is the smoothest path on
    // browsers that expose front/rear switching through applyConstraints().
    if (videoTrack && typeof videoTrack.applyConstraints === "function") {
        try {
            ...
            // Fall through to a fresh video track.
        }
    }

    // Some mobile browsers cannot switch an active track. Keep the microphone
    // alive and replace only the video track so the Live session is not lost.
    const oldVideo = videoTrack;
    ...
    try {
        ...
        // A few desktop/webview implementations reject `exact`; retry with
        // `ideal` before reporting a genuine failure.
        newVideoStream = await navigator.mediaDevices.getUserMedia({
            video: {
    ...
    if (!newVideoTrack) throw new Error("The browser returned no video track.");
...
async function switchLiveCamera() {
    ...
    if (button) {
        ...
        button.innerHTML = '<span class="flip-spinner" aria-hidden="true"></span>';
    }
    ...
    try {
        ...
        const result = await acquireCameraForFacing(nextFacing);
        ...
        // If a new MediaStream was created, stop the old audio/video tracks
        // only after the replacement has successfully started.
        if (result !== oldStream) {
            const replacementVideo = result.getVideoTracks()[0];
            if (oldVideoTrack && oldVideoTrack !== replacementVideo) {
                try { oldVideoTrack.stop(); } catch (_) {}
            }
            await rebindLiveAudioToStream(cameraStream);
        }
        ...
        liveFacingMode = liveFacingMode === "user" ? "user" : "environment";
        liveSetState("Could not switch camera", false);
        showPasteHint("Camera switch failed — keeping the current camera");
    } finally {
        if (button) {
            button.disabled = false;
            button.removeAttribute("aria-busy");
            button.innerHTML = '<span class="flip-icon" aria-hidden="true">↔</span>';
        }
        updateLiveControls();
    }
}
...
/* =========================================================
   ASRIT LIQUID GLASS 3.0 — FINAL UI REPLACEMENT
   Replaces the old static/peach visual layer with a light,
   water-like iOS-inspired glass surface and fluid motion.
========================================================= */
:root{
  --lg-ink:#172534;
  --lg-muted:#6b7b8b;
  --lg-line:rgba(55,82,101,.13);
  --lg-white:rgba(255,255,255,.68);
  --lg-white-strong:rgba(255,255,255,.84);
  --lg-shadow:0 18px 60px rgba(54,92,118,.12);
  --lg-spring:cubic-bezier(.22,1,.36,1);
  --lg-smooth:cubic-bezier(.25,.8,.25,1);
}

html,body{
  background:
    radial-gradient(900px 650px at 8% 2%,rgba(255,255,255,.98),transparent 62%),
    radial-gradient(850px 650px at 94% 10%,rgba(190,232,255,.70),transparent 60%),
    radial-gradient(850px 650px at 72% 100%,rgba(207,242,255,.78),transparent 62%),
    linear-gradient(145deg,#f8fcff 0%,#eaf7ff 48%,#d8effa 100%) !important;
  color:var(--lg-ink) !important;
}
...
body::before,body::after{
  ...
  animation:liquidDrift 16s ease-in-out infinite alternate !important;
}
body::before{left:-12vw;top:-12vw;background:radial-gradient(circle,rgba(255,255,255,.95),rgba(164,222,255,.30) 48%,transparent 72%);}
body::after{right:-14vw;bottom:-15vw;background:radial-gradient(circle,rgba(178,231,255,.62),rgba(255,255,255,.32) 48%,transparent 72%);animation-delay:-7s !important;}

@keyframes liquidDrift{
  from{transform:translate3d(0,0,0) scale(1);}
  to{transform:translate3d(3vw,2vh,0) scale(1.08);}
}
@keyframes liquidEnter{from{opacity:0;transform:translate3d(0,14px,0) scale(.985);filter:blur(2px)}to{opacity:1;transform:none;filter:blur(0)}}
@keyframes liquidPop{0%{transform:scale(.96);opacity:0}70%{transform:scale(1.015)}100%{transform:scale(1);opacity:1}}
@keyframes liquidSheen{0%{transform:translateX(-130%);opacity:0}30%{opacity:.22}70%{opacity:.08}100%{transform:translateX(130%);opacity:0}}
@keyframes flipSpin{to{transform:rotate(180deg)}}

/* Remove the old opaque/peach surfaces. */
.sidebar,.topbar,.input-glass,.glass-button,.icon-button,.search,
.camera-box,.workspace-panel,.image-studio-card,.quick-card,
.message.assistant .message-bubble,.message.user .message-bubble,
.mobile-dock{
  background:linear-gradient(145deg,rgba(255,255,255,.76),rgba(255,255,255,.38)) !important;
  border-color:rgba(255,255,255,.78) !important;
  box-shadow:var(--lg-shadow),inset 0 1px 0 rgba(255,255,255,.95),inset 0 -1px 0 rgba(90,130,155,.06) !important;
  -webkit-backdrop-filter:blur(30px) saturate(155%) !important;
  backdrop-filter:blur(30px) saturate(155%) !important;
  color:var(--lg-ink) !important;
}

.sidebar{animation:liquidEnter .48s var(--lg-spring) both !important;}
.main{position:relative;z-index:1;}
.topbar{animation:liquidEnter .46s var(--lg-spring) .03s both !important;}

.nav-item,.history-item,.history-main,.status,.history-title,.logo-title,
.logo-sub,.welcome h1,.welcome p,.message-bubble,#messageInput{color:var(--lg-ink) !important;}
.history-title,.logo-sub,.welcome p,.status,.live-state{color:var(--lg-muted) !important;}
.search::placeholder,#messageInput::placeholder{color:#7890a1 !important;}

/* Fluid scrolling / momentum */
.chat,.history,.feature-content,.ios-docs-list,.history-screen-list{
  scroll-behavior:smooth !important;
  overscroll-behavior:contain;
  -webkit-overflow-scrolling:touch;
  scrollbar-gutter:stable;
}
.chat{scroll-padding-block:28px;}
.chat::-webkit-scrollbar,.history::-webkit-scrollbar{width:8px;}
.chat::-webkit-scrollbar-thumb,.history::-webkit-scrollbar-thumb{background:rgba(67,108,133,.18);border-radius:99px;border:2px solid transparent;background-clip:padding-box;}

/* Natural message arrival. */
.message{animation:liquidEnter .42s var(--lg-spring) both;}
.message-bubble{transition:transform .32s var(--lg-spring),box-shadow .32s var(--lg-smooth),background .32s ease !important;}
.message-bubble:hover{transform:translateY(-1px);box-shadow:0 16px 38px rgba(56,101,127,.13),inset 0 1px 0 rgba(255,255,255,.95) !important;}

/* Glass controls with iOS-like press physics. */
button,.glass-button,.nav-item,.history-item,.icon-button,.quick-card,
.ios-app-icon,.ios-widget,.dock-btn{transition:transform .24s var(--lg-spring),box-shadow .28s var(--lg-smooth),background .24s ease,border-color .24s ease,opacity .2s ease !important;will-change:transform;}
button::after,.glass-button::after{animation:none !important;}
button:hover,.glass-button:hover{transform:translateY(-2px) scale(1.015) !important;}
button:active,.glass-button:active{transform:scale(.965) translateY(1px) !important;}

.glass-button,.icon-button{position:relative;overflow:hidden;}
.glass-button::before,.icon-button::before{
  content:"";position:absolute;inset:0;border-radius:inherit;pointer-events:none;
  background:linear-gradient(115deg,transparent 22%,rgba(255,255,255,.45) 50%,transparent 78%);
  transform:translateX(-130%);transition:transform .65s var(--lg-smooth);
}
.glass-button:hover::before,.icon-button:hover::before{transform:translateX(130%);}

.input-area{z-index:60 !important;}
.input-glass{
  border-radius:26px !important;
  padding:8px !important;
  min-height:66px !important;
  box-shadow:0 18px 60px rgba(48,91,119,.15),inset 0 1px 0 rgba(255,255,255,.98) !important;
}

/* Welcome orb becomes a refractive water lens. */
.big-orb,.logo-orb{
  background:radial-gradient(circle at 32% 25%,rgba(255,255,255,.98),rgba(208,240,255,.72) 42%,rgba(140,211,242,.55) 70%,rgba(255,255,255,.62)) !important;
  border:1px solid rgba(255,255,255,.88) !important;
  color:#2c627e !important;
  box-shadow:0 22px 70px rgba(58,148,193,.18),inset 0 2px 0 rgba(255,255,255,.95) !important;
  animation:liquidPop .7s var(--lg-spring) both,liquidFloat 5s ease-in-out 1s infinite alternate !important;
}
@keyframes liquidFloat{to{transform:translateY(-6px) scale(1.015)}}

/* Live surface — more like a real call, not a basic modal. */
.camera-panel{
  background:rgba(230,243,250,.48) !important;
  -webkit-backdrop-filter:blur(36px) saturate(135%) !important;
  backdrop-filter:blur(36px) saturate(135%) !important;
  animation:liquidEnter .35s var(--lg-spring) both !important;
}
.camera-box{
  ...
  background:linear-gradient(145deg,rgba(255,255,255,.78),rgba(255,255,255,.42)) !important;
}
#camera{
  border-radius:27px !important;
  background:#dbeaf1 !important;
  box-shadow:inset 0 0 0 1px rgba(255,255,255,.72),0 16px 50px rgba(34,77,99,.12) !important;
  transition:opacity .28s ease,transform .38s var(--lg-spring) !important;
}
.camera-controls{align-items:center !important;}
.camera-controls .glass-button{
  min-height:48px !important;
...
#liveStatusPill{padding:6px 10px;border-radius:99px;background:rgba(255,255,255,.54);box-shadow:inset 0 1px 0 white;color:#3b8c64 !important;}
#liveStatusPill::first-letter{color:#3ed07e;}
.flip-icon{display:inline-block;font-size:20px;line-height:1;}
.flip-spinner{display:block;width:18px;height:18px;border:2px solid rgba(23,37,52,.18);border-top-color:#4aa9d2;border-radius:50%;animation:flipSpin .7s linear infinite;}

/* iOS-like translucent home cards. */
.ios-home-screen,.ios-docs-screen{background:transparent !important;}
.ios-app-icon,.ios-widget,.ios-home-dock{box-shadow:var(--lg-shadow),inset 0 1px 0 rgba(255,255,255,.96) !important;}
.ios-app-icon:hover,.ios-widget:hover{transform:translateY(-4px) scale(1.015) !important;}

/* Do not let the old "performance mode" kill the new motion. */
@media (prefers-reduced-motion:reduce){
  body::before,body::after,.message,.big-orb,.logo-orb,.camera-panel,.sidebar,.topbar{animation:none !important;}
  *,*::before,*::after{scroll-behavior:auto !important;transition-duration:.01ms !important;}
}
@media(max-width:800px){
  .sidebar{box-shadow:18px 0 70px rgba(43,88,116,.18) !important;}
  .chat{scroll-padding-bottom:110px;}
  .camera-panel{padding:10px !important;align-items:flex-end !important;}
  .camera-box{width:100% !important;height:min(88vh,760px) !important;border-radius:30px !important;}
  #camera{border-radius:23px !important;}
  .camera-controls{grid-template-columns:repeat(3,minmax(0,1fr)) !important;}
}
\n\n/* =========================================================\n   ASRIT LIQUID GLASS 4.0 — REFERENCE MATCH LAYER
   Stronger white glass, depth, refraction and fluid motion.
========================================================= */
:root{
  --lg4-ink:#111820;
  --lg4-muted:#69747d;
  --lg4-blue:#67c9ef;
  --lg4-cyan:#bcefff;
  --lg4-glass:rgba(255,255,255,.46);
  --lg4-glass-strong:rgba(255,255,255,.68);
  --lg4-edge:rgba(255,255,255,.88);
  --lg4-shadow:0 28px 90px rgba(57,79,92,.14);
  --lg4-spring:cubic-bezier(.16,1,.3,1);
}

html,body{
  background:#f8f8f6 !important;
  color:var(--lg4-ink) !important;
}
body{
  position:relative !important;
  overflow:hidden !important;
  background:
    radial-gradient(650px 500px at 14% 18%,rgba(220,246,255,.72),transparent 70%),
    radial-gradient(700px 520px at 84% 16%,rgba(245,250,255,.96),transparent 68%),
    radial-gradient(700px 520px at 74% 90%,rgba(207,240,252,.72),transparent 70%),
    linear-gradient(135deg,#ffffff 0%,#f8faf9 45%,#eef7fa 100%) !important;
}

/* Large moving liquid fields behind the interface. */
body::before,body::after{
  content:"" !important;
  display:block !important;
  position:fixed !important;
  ...
  pointer-events:none !important;
  border-radius:50% !important;
  filter:blur(26px) saturate(135%) !important;
  mix-blend-mode:normal !important;
}
body::before{
  width:46vw;height:46vw;left:-16vw;top:-18vw;
  background:radial-gradient(circle at 58% 52%,rgba(255,255,255,.98) 0 18%,rgba(171,231,252,.56) 38%,rgba(151,219,245,.18) 57%,transparent 72%);
  animation:lg4BlobA 15s ease-in-out infinite alternate !important;
}
body::after{
  width:50vw;height:50vw;right:-20vw;bottom:-22vw;
  background:radial-gradient(circle at 42% 40%,rgba(255,255,255,.94) 0 18%,rgba(178,229,247,.55) 39%,rgba(185,231,250,.15) 58%,transparent 73%);
  animation:lg4BlobB 19s ease-in-out -6s infinite alternate !important;
}
@keyframes lg4BlobA{0%{transform:translate3d(0,0,0) scale(.94)}50%{transform:translate3d(5vw,2vh,0) scale(1.08)}100%{transform:translate3d(1vw,6vh,0) scale(1)}}
@keyframes lg4BlobB{0%{transform:translate3d(0,0,0) scale(1)}50%{transform:translate3d(-4vw,-3vh,0) scale(1.1)}100%{transform:translate3d(-1vw,2vh,0) scale(.96)}}

/* Extra floating droplets without adding HTML elements. */
.app::before,.main::after{
  content:"";position:fixed;z-index:0;pointer-events:none;border-radius:50%;
  border:1px solid rgba(255,255,255,.9);
  background:radial-gradient(circle at 32% 28%,rgba(255,255,255,.95),rgba(215,246,255,.34) 45%,rgba(255,255,255,.08) 70%,transparent 72%);
  box-shadow:inset 2px 2px 8px rgba(255,255,255,.9),inset -3px -4px 9px rgba(116,176,202,.10),0 18px 45px rgba(88,143,164,.10);
  backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);
}
.app::before{width:74px;height:74px;left:30%;top:15%;animation:lg4Bubble1 10s ease-in-out infinite alternate}
.main::after{width:54px;height:54px;right:10%;bottom:19%;animation:lg4Bubble2 13s ease-in-out -4s infinite alternate}
@keyframes lg4Bubble1{to{transform:translate3d(20px,26px,0) scale(1.08)}}
@keyframes lg4Bubble2{to{transform:translate3d(-18px,-24px,0) scale(.92)}}

.app{position:relative !important;z-index:2 !important;gap:18px !important;padding:16px !important;}
.sidebar{
  position:relative !important;z-index:10 !important;width:286px !important;padding:18px !important;
  border:1px solid rgba(255,255,255,.88) !important;border-radius:32px !important;
  border-right:1px solid rgba(255,255,255,.88) !important;
  background:linear-gradient(150deg,rgba(255,255,255,.70),rgba(255,255,255,.36)) !important;
  box-shadow:var(--lg4-shadow),inset 1px 1px 0 rgba(255,255,255,.98),inset -1px -1px 0 rgba(123,170,190,.07) !important;
  backdrop-filter:blur(42px) saturate(170%) !important;-webkit-backdrop-filter:blur(42px) saturate(170%) !important;
  overflow:hidden !important;
}
.sidebar::before,.topbar::before,.input-glass::before,.quick-card::before,.camera-box::before{
  content:"";position:absolute;inset:0;pointer-events:none;border-radius:inherit;
  background:linear-gradient(115deg,rgba(255,255,255,.62),transparent 24%,transparent 66%,rgba(255,255,255,.24));
  opacity:.75;
}
.logo{padding:4px 8px 14px !important;position:relative;z-index:1}
.logo-orb{
  width:58px !important;height:58px !important;border-radius:50% !important;
  background:radial-gradient(circle at 35% 28%,#fff 0 18%,rgba(224,247,255,.78) 40%,rgba(174,226,246,.48) 65%,rgba(255,255,255,.55)) !important;
  border:1px solid rgba(255,255,255,.96) !important;color:#354b58 !important;font-family:Georgia,serif !important;font-size:28px !important;
  box-shadow:inset 2px 2px 8px rgba(255,255,255,.96),inset -5px -6px 13px rgba(89,143,166,.10),0 14px 38px rgba(70,112,130,.12) !important;
  animation:lg4LogoFloat 6s ease-in-out infinite alternate !important;
}
@keyframes lg4LogoFloat{to{transform:translateY(-4px) rotate(.8deg)}}
.logo-title{font-size:20px !important;letter-spacing:.16em !important;color:#111820 !important}
.logo-sub{color:#7a858c !important;letter-spacing:.10em !important}

.new-chat,.search,.nav-item,.history-item{
  position:relative !important;z-index:1 !important;
  border:1px solid rgba(255,255,255,.88) !important;
  background:linear-gradient(135deg,rgba(255,255,255,.70),rgba(255,255,255,.34)) !important;
  color:#1b252d !important;
  box-shadow:0 8px 28px rgba(72,102,115,.08),inset 0 1px 0 rgba(255,255,255,.96) !important;
  backdrop-filter:blur(24px) saturate(150%) !important;-webkit-backdrop-filter:blur(24px) saturate(150%) !important;
}
.new-chat{min-height:48px !important;border-radius:18px !important}
.search{border-radius:17px !important;color:#17222a !important}
.nav-item{min-height:48px !important;margin:6px 0 !important;border-radius:18px !important}
.nav-item:hover,.history-item:hover{background:rgba(255,255,255,.78) !important;transform:translateX(3px) !important}
.nav-item.active{background:linear-gradient(135deg,rgba(255,255,255,.92),rgba(219,244,252,.58)) !important;box-shadow:0 10px 30px rgba(75,128,150,.12),inset 0 1px 0 #fff !important}

.main{position:relative !important;z-index:3 !important;min-width:0 !important;overflow:hidden !important;border-radius:32px !important;border:1px solid rgba(255,255,255,.84) !important;background:rgba(255,255,255,.24) !important;box-shadow:var(--lg4-shadow),inset 0 1px 0 rgba(255,255,255,.92) !important;backdrop-filter:blur(26px) saturate(135%) !important;-webkit-backdrop-filter:blur(26px) saturate(135%) !important}
.topbar{height:72px !important;position:relative !important;border:0 !important;border-bottom:1px solid rgba(255,255,255,.68) !important;background:linear-gradient(180deg,rgba(255,255,255,.64),rgba(255,255,255,.30)) !important;backdrop-filter:blur(30px) saturate(155%) !important;-webkit-backdrop-filter:blur(30px) saturate(155%) !important}
.status{color:#68747c !important;position:relative;z-index:2}.status-dot{box-shadow:0 0 16px rgba(44,205,134,.55) !important}

.chat{position:relative;z-index:1 !important;background:transparent !important}
.welcome{min-height:calc(100% - 20px) !important;padding-top:26px !important}
.big-orb{
  width:126px !important;height:126px !important;border-radius:50% !important;
  background:radial-gradient(circle at 32% 25%,#fff 0 18%,rgba(231,249,255,.78) 38%,rgba(183,231,247,.42) 64%,rgba(255,255,255,.48)) !important;
  border:1px solid rgba(255,255,255,.94) !important;color:#63727b !important;font-family:Georgia,serif !important;font-size:58px !important;font-weight:400 !important;
  box-shadow:inset 3px 3px 13px rgba(255,255,255,.98),inset -7px -8px 17px rgba(82,145,169,.10),0 24px 65px rgba(72,128,150,.16) !important;
  animation:lg4Orb 7s ease-in-out infinite alternate !important;
}
@keyframes lg4Orb{to{transform:translateY(-7px) rotate(1deg) scale(1.025);box-shadow:inset 3px 3px 13px rgba(255,255,255,.98),inset -7px -8px 17px rgba(82,145,169,.12),0 30px 78px rgba(72,128,150,.20)}}
.welcome h1{font-size:clamp(48px,6vw,74px) !important;letter-spacing:-.055em !important;color:#111820 !important;margin:28px 0 2px !important}
.welcome-tagline,.welcome p{color:#727d84 !important;font-size:17px !important}
.quick-actions{gap:18px !important;margin-top:38px !important}
.quick-card{
  position:relative !important;min-width:280px !important;min-height:116px !important;padding:22px !important;border-radius:28px !important;
  border:1px solid rgba(255,255,255,.90) !important;
  background:linear-gradient(145deg,rgba(255,255,255,.68),rgba(255,255,255,.34)) !important;
  box-shadow:0 18px 55px rgba(64,106,124,.11),inset 0 1px 0 rgba(255,255,255,.98),inset 0 -1px 0 rgba(93,145,163,.06) !important;
  backdrop-filter:blur(34px) saturate(160%) !important;-webkit-backdrop-filter:blur(34px) saturate(160%) !important;
}
.quick-card:hover{transform:translateY(-7px) scale(1.018) !important;box-shadow:0 26px 70px rgba(64,106,124,.16),inset 0 1px 0 #fff !important}
.quick-card strong{font-size:17px !important;color:#121a20 !important}.quick-card small{color:#748087 !important}.reference-card-icon{width:42px !important;height:42px !important;border-radius:50% !important;background:rgba(255,255,255,.66) !important;border:1px solid rgba(255,255,255,.92) !important;box-shadow:inset 0 1px 0 #fff,0 7px 20px rgba(70,110,126,.08) !important}

.input-area{position:relative !important;z-index:20 !important;padding:12px 7% 20px !important;background:linear-gradient(180deg,transparent,rgba(246,252,254,.42)) !important}
.input-glass{
  position:relative !important;overflow:hidden !important;min-height:72px !important;padding:9px !important;border-radius:38px !important;
  border:1px solid rgba(255,255,255,.94) !important;
  background:linear-gradient(145deg,rgba(255,255,255,.72),rgba(255,255,255,.40)) !important;
  box-shadow:0 22px 65px rgba(58,99,117,.15),inset 0 2px 0 rgba(255,255,255,.98),inset 0 -1px 0 rgba(82,136,156,.08) !important;
  backdrop-filter:blur(38px) saturate(175%) !important;-webkit-backdrop-filter:blur(38px) saturate(175%) !important;
}
.input-glass::after{content:"";position:absolute;left:-35%;top:-60%;width:32%;height:220%;background:linear-gradient(100deg,transparent,rgba(255,255,255,.62),transparent);transform:rotate(12deg);animation:lg4Sheen 7s ease-in-out infinite !important;pointer-events:none}
@keyframes lg4Sheen{0%,55%{transform:translateX(-160%) rotate(12deg);opacity:0}64%{opacity:.55}78%,100%{transform:translateX(500%) rotate(12deg);opacity:0}}
.input-glass .add-button,.input-glass #micButton,.input-glass .send-button{border:1px solid rgba(255,255,255,.92) !important;background:rgba(255,255,255,.62) !important;color:#1b252d !important;box-shadow:0 8px 24px rgba(68,106,122,.10),inset 0 1px 0 #fff !important}
.input-glass .send-button{background:linear-gradient(145deg,rgba(255,255,255,.88),rgba(175,231,250,.78)) !important}
#messageInput{color:#17222a !important}#messageInput::placeholder{color:#7b878d !important}

/* All workspaces/modes use the same glass language. */
.image-studio,.history-screen,.feature-screen,.ios-home-screen,.ios-docs-screen{background:rgba(248,252,253,.54) !important;backdrop-filter:blur(34px) saturate(150%) !important;-webkit-backdrop-filter:blur(34px) saturate(150%) !important}
.image-studio-card,.feature-shell,.history-shell,.ios-home-screen>div{border:1px solid rgba(255,255,255,.90) !important;background:linear-gradient(145deg,rgba(255,255,255,.72),rgba(255,255,255,.38)) !important;box-shadow:var(--lg4-shadow),inset 0 1px 0 #fff !important;backdrop-filter:blur(34px) saturate(155%) !important;-webkit-backdrop-filter:blur(34px) saturate(155%) !important}
.workspace-row,.project-card,.ios-app-icon,.ios-widget{background:rgba(255,255,255,.54) !important;border-color:rgba(255,255,255,.86) !important;box-shadow:0 12px 35px rgba(69,105,120,.08),inset 0 1px 0 #fff !important}

/* Live: premium glass call sheet + clearer connection status. */
.camera-panel{background:rgba(25,35,40,.44) !important;backdrop-filter:blur(26px) saturate(120%) !important;-webkit-backdrop-filter:blur(26px) saturate(120%) !important}
.camera-box{position:relative !important;width:min(1120px,95vw) !important;height:min(790px,92vh) !important;padding:14px !important;border-radius:38px !important;border:1px solid rgba(255,255,255,.86) !important;background:linear-gradient(145deg,rgba(255,255,255,.78),rgba(255,255,255,.50)) !important;box-shadow:0 40px 110px rgba(0,0,0,.22),inset 0 1px 0 rgba(255,255,255,.96) !important;backdrop-filter:blur(38px) saturate(150%) !important;-webkit-backdrop-filter:blur(38px) saturate(150%) !important}
#camera{border-radius:30px !important;box-shadow:inset 0 0 0 1px rgba(255,255,255,.72),0 18px 55px rgba(0,0,0,.14) !important}
.live-meta{color:#151b20 !important}.live-state{color:#66727a !important}
.camera-controls .glass-button{border:1px solid rgba(255,255,255,.92) !important;background:linear-gradient(145deg,rgba(255,255,255,.80),rgba(255,255,255,.52)) !important;color:#17222a !important;box-shadow:0 10px 28px rgba(34,57,66,.10),inset 0 1px 0 #fff !important}

/* Restore actual animation after the legacy global kill-switch. */
*,*::before,*::after{animation-duration:initial !important}
@media (prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.001ms !important;animation-iteration-count:1 !important;transition-duration:.001ms !important}}
.live-launch-button{
  border-radius:999px !important;
  padding:12px 19px !important;
  background:linear-gradient(145deg,rgba(255,255,255,.84),rgba(216,244,253,.58)) !important;
  box-shadow:0 10px 30px rgba(62,108,126,.12),inset 0 1px 0 #fff !important;
}
.live-launch-button:hover{box-shadow:0 16px 42px rgba(62,108,126,.18),inset 0 1px 0 #fff !important}
#liveStatusPill{white-space:nowrap !important;max-width:min(70vw,520px) !important;overflow:hidden !important;text-overflow:ellipsis !important}
.live-state{max-width:min(85vw,760px) !important;margin-inline:auto !important;line-height:1.45 !important}

@media(max-width:800px){
  .app{padding:8px !important;gap:8px !important}.sidebar{border-radius:26px !important}.main{border-radius:26px !important}
  .quick-actions{grid-template-columns:1fr !important;width:min(92vw,420px) !important}.quick-card{min-width:0 !important}
  .input-area{padding:8px 4% 14px !important}.input-glass{border-radius:30px !important}
}
