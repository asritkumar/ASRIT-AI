...
def live_google_url():
    return f"{LIVE_WS_URL}?key={GEMINI_API_KEY}"
...
async def live_websocket(websocket: WebSocket):
    ...
    try:
        ...
        browser_to_google = asyncio.create_task(
            relay_browser_to_google(websocket, google_ws)
        )
        google_to_browser = asyncio.create_task(
            relay_google_to_browser(websocket, google_ws)
        )

        done, pending = await asyncio.wait(
            {browser_to_google, google_to_browser},
        ...
        try:
            await websocket.send_json({
                "type": "error",
                "message": f"Live connection error: {error}"
            })
        except Exception:
            pass
    finally:
...
async def upload_file(
    ...
    safe_name = (
        datetime.now().strftime("%Y%m%d_%H%M%S_%f_")
        + original_filename
    )
    ...
    add_message(
        conversation_id,
        "user",
        f"[Uploaded file: {original_filename}] {message}"
    )
...
/* =========================================================
   SINGLE FRAME CAMERA ANALYSIS
========================================================= */

async function analyzeCamera() {
    // Kept as a compatibility function for the original app.
    if (!cameraStream) {
        await openCamera();
        return;
    }
    ...
    canvas.toBlob(async function(blob) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.detail || data.answer || "Camera request failed");
            }
