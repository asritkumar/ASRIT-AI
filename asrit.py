...
Answer naturally and directly as ASRIT.
"""
...
async def _generate_image_rest(prompt):
    """REST fallback for Gemini Interactions image generation."""
...
async def generate_image(prompt, conversation_id=0):
    ...
    try:
        ...
        # Current Gemini image API: Interactions + Nano Banana 2.
        if client is not None and hasattr(client, "interactions"):
            try:
                ...
                print("Image SDK fallback:", repr(sdk_error))
        ...
        image_data = None
        output_image = (
            interaction.get("output_image")
            if isinstance(interaction, dict)
            else getattr(interaction, "output_image", None)
        )
        ...
        # Fallback for interleaved output blocks.
        if not image_data:
        ...
        if not image_data and isinstance(interaction, dict):
            for block in interaction.get("output", []) or []:
                if isinstance(block, dict) and block.get("type") == "image" and block.get("data"):
                    image_data = block["data"]
                    break

        if not image_data:
            return {"error": "Gemini returned no image data. Check that the API key/project can use gemini-3.1-flash-image."}
...
async def live_websocket(websocket: WebSocket):
    ...
    try:
        ...
        # Gemini 3.8 Live WebSocket setup.
        # The current WebSocket server expects responseModalities inside
        # generationConfig. Audio output is native 24 kHz PCM.
        setup = {
            "setup": {
...
/* Dedicated workspaces: never render tools inside the chat timeline */
.feature-screen{
  position:fixed; inset:0; z-index:4900; display:none; overflow:auto;
  padding:clamp(22px,4vw,52px);
  background:linear-gradient(145deg,#efc3a7 0%,#f6d8c5 48%,#e8b18f 100%);
  color:#211812;
}
.feature-screen.is-visible{display:block;animation:iosPageIn .28s var(--motion-spring) both}
.feature-shell{max-width:1120px;margin:0 auto;padding-bottom:40px}
.feature-screen-head{
  display:flex;align-items:flex-start;justify-content:space-between;gap:20px;
  margin-bottom:22px;
}
.feature-screen-head h2{margin:4px 0 0;font-size:clamp(28px,5vw,42px);letter-spacing:-1px;color:#17100c}
.feature-screen-head p{margin:7px 0 0;color:#624c40}
.feature-back{
  flex:0 0 auto;border:1px solid rgba(70,40,25,.12);border-radius:16px;
  padding:11px 16px;background:rgba(255,255,255,.62);color:#17100c;
  font-weight:700;cursor:pointer;box-shadow:0 8px 22px rgba(70,40,20,.08);
}
.feature-content{display:grid;gap:16px}
.feature-content .workspace-panel{display:contents}
.feature-content .workspace-head{
  display:flex;align-items:center;justify-content:space-between;gap:16px;
  padding:22px;border-radius:24px;background:rgba(255,255,255,.46);
  border:1px solid rgba(255,255,255,.66);box-shadow:0 16px 40px rgba(70,40,20,.08);
}
.feature-content .workspace-list{display:grid;gap:10px}
.feature-content .workspace-row,
.feature-content .project-card,
.feature-content .schedule-form{
  background:rgba(255,255,255,.58)!important;
  border:1px solid rgba(255,255,255,.72)!important;
  border-radius:20px!important;
  box-shadow:0 10px 28px rgba(70,40,20,.07)!important;
}
.feature-content .workspace-row{padding:16px!important}
.feature-content input,.feature-content textarea,.feature-content select{
  color:#17100c!important;background:rgba(255,255,255,.82)!important;
}
.feature-content .codex-grid{
  display:grid;grid-template-columns:1.35fr .85fr;gap:16px;
}
.feature-content .codex-grid>textarea,
.feature-content .codex-grid>div{
  min-height:420px;border-radius:24px!important;
}
.feature-content .codex-grid textarea{
  border:1px solid rgba(70,40,25,.13)!important;
  padding:18px!important;background:rgba(255,255,255,.72)!important;
}
.feature-content .codex-result{
  min-height:150px;padding:18px;border-radius:20px;
  background:rgba(25,18,14,.88);color:#fff;overflow:auto;
}
@media(max-width:700px){
  .feature-screen{padding:16px 13px 92px}
  .feature-screen-head{position:sticky;top:0;z-index:2;padding:4px 0 14px;margin-bottom:12px;
    background:linear-gradient(180deg,#f3cdb5 80%,transparent)}
  .feature-screen-head h2{font-size:28px}
  .feature-back{padding:10px 12px;font-size:13px}
  .feature-content .workspace-head{flex-direction:column;align-items:stretch;padding:17px}
  .feature-content .workspace-row{display:grid!important;grid-template-columns:auto 1fr!important;gap:10px!important}
  .feature-content .workspace-row button{grid-column:1/-1;width:100%}
  .feature-content .codex-grid{grid-template-columns:1fr}
  .feature-content .codex-grid>textarea,
  .feature-content .codex-grid>div{min-height:260px}
  .feature-content .schedule-form{padding:14px!important}
}
</style>


<style id="live-camera-switch-final">
#liveFlipCameraButton {
    min-width: 46px;
    min-height: 46px;
    display: inline-flex;
    align-items: center;
...
#liveFlipCameraButton:disabled { opacity: .6; cursor: wait; }
@media (max-width: 700px) {
    .camera-controls {
    ...
    .camera-controls .glass-button {
        min-height: 48px !important;
        width: 100% !important;
    }
    .camera-controls .glass-button:last-child {
        grid-column: 1 / -1;
    }
}
</style>

<style id="asrit-dark-text-final">
/* ASRIT final readable text/icon theme */
html, body,
.app, .main, .content, .workspace,
.sidebar, .side-nav, .nav-item, .mobile-dock,
.topbar, .chat-area, .chat-shell,
.history-screen, .history-shell,
.feature-screen, .feature-shell,
.card, .glass-card, .panel, .workspace-card,
.modal, .dialog, .menu, .dropdown {
    color: #171717 !important;
}

/* All common text-bearing controls */
button, input, textarea, select,
label, p, h1, h2, h3, h4, h5, h6,
span, a, small, strong, b, li,
.nav-item, .mobile-dock button,
...
/* Icons/symbols should never inherit white from the glass theme */
.nav-icon, .mobile-icon, .icon, .symbol,
...
/* Response action buttons: dark icon/text on light glass */
...
/* Avoid white-on-white active/hover states */
.nav-item.active,
.nav-item:hover,
...
/* Preserve intentional green online indicator */
.online-dot, .status-dot {
    color: #20c98a !important;
    background: #20c98a !important;
}

/* Dark readable divider/border for controls */
input, textarea, select,
.search, .composer input, .composer textarea {
    border-color: rgba(23,23,23,.16) !important;
}

/* Mobile workspace headings/content */
@media (max-width: 700px) {
    .workspace, .feature-screen, .history-screen,
    .feature-screen *, .history-screen * {
        color: #171717 !important;
    }
}
</style>

<style id="asrit-final-black-icons-modern-actions">
/* =========================================================
   FINAL ASRIT READABILITY + MODERN RESPONSE ACTIONS
   ========================================================= */

/* Force dark text and symbols throughout the light/glass UI. */
body,
button, input, textarea, select,
.nav-item, .dock-btn,
.nav-item *, .dock-btn *,
...
/* Menu button: black bars, not white. */
...
/* Live microphone: pure dark CSS icon, no colored emoji. */
.live-mic-icon {
...
.live-mic-icon::after {
    ...
    transform: translateX(-50%) !important;
    background: #111827 !important;
    box-shadow: -5px 5px 0 -0.5px #111827, 5px 5px 0 -0.5px #111827 !important;
}

/* Live controls: dark icons/text. */
.camera-controls .glass-button,
.camera-controls .glass-button *,
#liveFlipCameraButton {
    color: #111827 !important;
...
/* Modern response actions: pill-style, not basic icon buttons. */
...
.message.assistant .response-action {
    ...
    color: #111827 !important;
    box-shadow: 0 3px 12px rgba(17,24,39,.07), inset 0 1px 0 rgba(255,255,255,.95) !important;
    font: 600 12px/1 system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif !important;
    cursor: pointer !important;
    transition: transform .16s ease, background .16s ease, box-shadow .16s ease !important;
}

.message.assistant .response-action:hover {
    background: #ffffff !important;
    color: #111827 !important;
    transform: translateY(-1px) !important;
    box-shadow: 0 7px 18px rgba(17,24,39,.11) !important;
}

.message.assistant .response-action:active {
    transform: scale(.97) !important;
}

.message.assistant .response-action .ra-label {
    color: #111827 !important;
    font-weight: 650 !important;
}

.message.assistant .response-action .ra-icon {
    position: relative !important;
...
/* Remove old response-action states from ever appearing. */
.response-action[data-action="like"],
.response-action[data-action="dislike"],
.response-action[data-action="retry"],
.response-action[data-action="more"] {
    ...
    <main class="main">
        ...
        <header class="topbar">
            ...
            <div class="topbar-tools">
                <button id="taskViewButton" class="utility-button" onclick="openTaskView()" title="Task View" aria-label="Task View">▦</button>
                <button id="themeButton" class="utility-button" onclick="toggleDarkMode()" title="Dark mode" aria-label="Dark mode">◐</button>
                <div class="region-clock" title="Browser region time">
                    <strong id="regionTime">--:--</strong>
                    <span id="regionDate">--/--/----</span>
                    <small id="regionName">Local</small>
                </div>
            </div>
            <button
                class="glass-button live-launch-button"
                onclick="openCamera()"
            >
                <span class="live-dot-icon"></span> Live
            </button>
...
<nav class="mobile-dock" aria-label="ASRIT quick navigation">
    ...
    <button class="dock-btn" onclick="showMode('chat')"><span>◌</span>Chat</button>
...
<!-- Dedicated feature workspace: Library / Scheduled / Plugins / Projects / Codex -->
<section id="featureScreen" class="feature-screen" aria-label="ASRIT workspace">
  <div class="feature-shell">
    <div class="feature-screen-head">
      <div>
        <span id="featureEyebrow" class="eyebrow">ASRIT WORKSPACE</span>
        <h2 id="featureTitle">Workspace</h2>
        <p id="featureSubtitle">A dedicated ASRIT workspace.</p>
      </div>
      <button class="feature-back" type="button" onclick="closeFeatureScreen()">‹ Back to Chat</button>
    </div>
    <div id="featureContent" class="feature-content"></div>
  </div>
</section>


<!-- ASRIT attachment launcher -->
<div id="composerMenu" class="composer-menu" aria-hidden="true">
    <div class="composer-menu-head">
        <div><b>Add anything</b><small>Upload from this device</small></div>
        <button type="button" onclick="closeComposerMenu()">×</button>
    </div>
    <button class="composer-menu-item" onclick="chooseFiles('all')"><span>⌕</span><div><b>Photos & files</b><small>Images, PDF, documents, video and more</small></div></button>
    <button class="composer-menu-item" onclick="chooseFiles('photo')"><span>▧</span><div><b>Photo from device</b><small>Camera roll / gallery / desktop image</small></div></button>
    <button class="composer-menu-item" onclick="openCamera()"><span>◉</span><div><b>Camera</b><small>Use the camera directly</small></div></button>
    <button class="composer-menu-item" onclick="showMode('images');closeComposerMenu()"><span>✦</span><div><b>Create image</b><small>Generate a new image with ASRIT</small></div></button>
</div>

<div id="taskViewPanel" class="task-view-panel" aria-hidden="true">
    <div class="task-view-head"><div><span>ASRIT TASK VIEW</span><h2>Active workspace</h2></div><button onclick="closeTaskView()">×</button></div>
    <div class="task-grid">
        <button class="task-card" onclick="showMode('chat');closeTaskView()"><span>◌</span><b>Chat</b><small>Current conversation</small></button>
        <button class="task-card" onclick="showMode('images');closeTaskView()"><span>✦</span><b>Images</b><small>Creative studio</small></button>
        <button class="task-card" onclick="openCamera()"><span>◉</span><b>ASRIT Live</b><small id="taskLiveState">Ready</small></button>
        <button class="task-card" onclick="openFeature('library');closeTaskView()"><span>▤</span><b>Library</b><small>Uploaded files</small></button>
        <button class="task-card" onclick="openFeature('scheduled');closeTaskView()"><span>◷</span><b>Scheduled</b><small>Tasks & reminders</small></button>
        <button class="task-card" onclick="openFeature('codex');closeTaskView()"><span>{ }</span><b>Codex</b><small>Coding workspace</small></button>
    </div>
</div>

<div id="networkToast" class="network-toast" aria-live="polite"><span class="network-icon">⌁</span><span id="networkToastText">Online</span></div>
...
>
    ...
    <div class="camera-box">
        ...
        <div class="camera-controls">
            <button id="liveMicButton" class="glass-button" onclick="toggleLiveMic()" title="Mute microphone" aria-label="Mute microphone"><span class="live-mic-icon" aria-hidden="true"></span></button>
            <button id="liveCameraButton" class="glass-button" onclick="toggleLiveCamera()" title="Pause camera">📷</button>
            <button id="liveFlipCameraButton" class="glass-button live-flip-camera" onclick="switchLiveCamera()" title="Switch front/rear camera" aria-label="Switch front/rear camera">↔</button>
...
function addMessage(
    ...
    if (role === "assistant") {
        ...
        actions.innerHTML = `
            <button class="response-action modern-copy-action" data-action="copy" title="Copy response" aria-label="Copy response">
                <span class="ra-icon ra-copy-icon" aria-hidden="true"></span>
                <span class="ra-label">Copy</span>
            </button>
            <button class="response-action modern-share-action" data-action="share" title="Share response" aria-label="Share response">
                <span class="ra-icon ra-share-icon" aria-hidden="true"></span>
...
function closeFeatureScreen(){
    ...
    if(messages && !messages.children.length) document.getElementById("welcome").style.display="flex";
    currentMode="chat";
    document.querySelectorAll(".nav-item").forEach(btn=>btn.classList.toggle("active",btn.dataset.mode==="chat"));
}
...
async function openFeature(name) {
    ...
    const eyebrow=document.getElementById("featureEyebrow");
    const subtitle=document.getElementById("featureSubtitle");
    const chat=document.getElementById("chat");
    const input=document.querySelector(".input-area");
    const studio=document.getElementById("imageStudio");
    ...
    if(history) history.classList.remove("is-visible");
    if(chat) chat.style.display="none";
    if(input) input.style.display="none";
    if(studio) studio.classList.remove("open");

    const meta={
      library:["ASRIT LIBRARY","Library","Your files, screenshots, documents, and generated assets."],
      scheduled:["ASRIT AUTOMATIONS","Scheduled","Create and manage reminders and tasks in a dedicated workspace."],
      plugins:["ASRIT TOOLS","Plugins","Manage ASRIT tools without mixing them into Chat."],
      projects:["ASRIT WORKSPACE","Projects","Keep project notes and code separate from your conversations."],
      codex:["ASRIT CODEX","Codex","A focused coding workspace for code, instructions, and results."]
    }[name] || ["ASRIT WORKSPACE","Workspace",""];

    eyebrow.textContent=meta[0]; title.textContent=meta[1]; subtitle.textContent=meta[2];
    content.innerHTML='<div class="workspace-loading">Loading…</div>';
    screen.classList.add("is-visible");
    currentMode=name;
    document.querySelectorAll(".nav-item").forEach(btn=>btn.classList.toggle("active",false));

    try{
      if(name==="library"){
        const data=await (await fetch("/api/library")).json();
        content.innerHTML=`<div class="workspace-head"><div><span class="eyebrow">FILES</span><h3>Everything you added to ASRIT</h3></div><button class="glass-button" onclick="document.getElementById('fileInput').click()">＋ Add file</button></div>`;
        const list=document.createElement("div"); list.className="workspace-list";
        list.innerHTML=data.length?data.map(x=>`<a class="workspace-row" href="${x.url}" target="_blank"><span>▤</span><div><strong>${escapeHtml(x.filename)}</strong><small>${escapeHtml(x.mime_type||"file")} · ${escapeHtml(x.created_at)}</small></div><b>↗</b></a>`).join(""):`<div class="empty-state">No files yet. Add a photo, document, or generated image.</div>`;
        content.appendChild(list); return;
      }

      if(name==="scheduled"){
        const data=await (await fetch("/api/schedules")).json();
        content.innerHTML=`<div class="workspace-head"><div><span class="eyebrow">AUTOMATIONS</span><h3>Scheduled tasks</h3><small>Create reminders without leaving Chat cluttered.</small></div></div><div class="schedule-form"><input id="schTitle" placeholder="Title"><input id="schWhen" type="datetime-local"><select id="schRepeat"><option value="once">Once</option><option value="daily">Daily</option><option value="weekly">Weekly</option></select><textarea id="schPrompt" placeholder="What should ASRIT remember or run?"></textarea><button class="glass-button send-button" onclick="createSchedule()">＋ Schedule</button></div><div id="scheduleList" class="workspace-list"></div>`;
        renderSchedules(data); return;
      }

      if(name==="plugins"){
        const data=await (await fetch("/api/plugins")).json();
        content.innerHTML=`<div class="workspace-head"><div><span class="eyebrow">TOOLS</span><h3>ASRIT Plugins</h3><small>Turn tools on or off independently from Chat.</small></div></div><div id="pluginList" class="workspace-list"></div>`;
      ...
      if(name==="projects"){
        ...
        content.innerHTML=`<div class="workspace-head"><div><span class="eyebrow">PROJECTS</span><h3>Project spaces</h3><small>Keep long-running work separate from your Chat history.</small></div><button class="glass-button" onclick="createProject()">＋ New project</button></div><div id="projectList" class="workspace-list"></div>`;
        renderProjects(data); return;
      }

      if(name==="codex"){
        content.innerHTML=`<div class="workspace-head"><div><span class="eyebrow">CODEX</span><h3>Focused coding workspace</h3><small>Paste code and describe exactly what you want changed.</small></div></div><div class="codex-grid"><textarea id="codexCode" placeholder="Paste your code here…"></textarea><div><textarea id="codexPrompt" placeholder="What should I build, debug, explain, or change?"></textarea><button class="glass-button send-button" onclick="runCodex()">Run Codex</button></div></div><pre id="codexResult" class="codex-result"></pre>`;
        return;
      }
    }catch(error){
      content.innerHTML=`<div class="empty-state">Could not load this workspace.<br><small>${escapeHtml(error.message||String(error))}</small></div>`;
    }
}
...
/* File input is handled by the ASRIT Liquid UI listener below. */
...
async function switchLiveCamera() {
    ...
    const button = document.getElementById("liveFlipCameraButton");
    ...
    liveSetState(
        nextFacing === "environment" ? "Switching to rear camera…" : "Switching to front camera…",
        true
    );
    ...
    try {
        ...
        // Reconnect microphone capture to the new MediaStream.
        if (liveProcessor) {
        ...
        if (liveMuteGain) {
            try { liveMuteGain.disconnect(); } catch (_) {}
            liveMuteGain = null;
        }
        ...
        updateLiveControls();
        liveSetState(
            liveFacingMode === "environment" ? "Rear camera active" : "Front camera active",
            true
        );

    } catch (error) {
        console.error("Camera switch failed:", error);
        liveSetState("Could not switch camera — keeping current camera", false);
        alert(
            "ASRIT could not switch cameras.\n\n" +
            (error.message || error) +
...
function updateLiveControls() {
    ...
    const flip = document.getElementById("liveFlipCameraButton");

    if (flip) {
        flip.title = liveFacingMode === "user"
...
<style id="asrit-liquid-ocean-2026">
/* =========================================================
   ASRIT — LIQUID OCEAN GLASS SYSTEM
========================================================= */
:root{
  --ocean-a:#dff8ff; --ocean-b:#8edcff; --ocean-c:#4bbce9;
  --ink:#0c2233; --muted:#587286;
  --glass:rgba(255,255,255,.42); --glass-hi:rgba(255,255,255,.64);
  --line:rgba(255,255,255,.66); --shadow:0 22px 70px rgba(20,105,145,.18);
}
html,body{background:#b9ecff!important;color:var(--ink)!important}
body{
  background:
    radial-gradient(1100px 700px at 18% 8%,rgba(255,255,255,.92),transparent 55%),
    radial-gradient(900px 650px at 92% 12%,rgba(121,222,255,.62),transparent 58%),
    linear-gradient(160deg,#dff9ff 0%,#aee8fb 42%,#58bfe7 100%)!important;
}
body:before{content:"";position:fixed;inset:auto -15% -34% -15%;height:70%;z-index:0;pointer-events:none;opacity:.9;
  background:
    radial-gradient(65% 55% at 15% 40%,rgba(255,255,255,.48),transparent 62%),
    radial-gradient(60% 50% at 70% 30%,rgba(255,255,255,.25),transparent 65%),
    repeating-radial-gradient(ellipse at 50% 100%,rgba(255,255,255,.20) 0 3px,transparent 4px 26px);
  filter:blur(3px);animation:asritSeaFlow 16s ease-in-out infinite alternate;}
body:after{content:"";position:fixed;inset:0;z-index:1;pointer-events:none;background:linear-gradient(115deg,transparent 20%,rgba(255,255,255,.18) 38%,transparent 52%);background-size:240% 100%;animation:asritShimmer 11s linear infinite}
@keyframes asritSeaFlow{0%{transform:translate3d(-2%,3%,0) scale(1)}50%{transform:translate3d(2%,-2%,0) scale(1.04)}100%{transform:translate3d(-1%,1%,0) scale(1.02)}}
@keyframes asritShimmer{0%{background-position:160% 0}100%{background-position:-60% 0}}
@keyframes asritLiquidPulse{0%,100%{transform:translateY(0);filter:saturate(125%)}50%{transform:translateY(-1px);filter:saturate(145%)}}
@keyframes asritSidebarIn{from{width:78px}to{width:285px}}
@keyframes asritSidebarOut{from{width:285px}to{width:78px}}
@keyframes asritMenuIn{from{opacity:0;transform:translateY(8px) scale(.97)}to{opacity:1;transform:none}}
@keyframes asritToast{0%{opacity:0;transform:translateY(10px)}15%,80%{opacity:1;transform:none}100%{opacity:0;transform:translateY(10px)}}
.app,.sidebar,.main,.topbar,.chat,.input-area,.input-glass,.quick-card,.glass-button,.workspace-panel,.image-studio-card,.camera-box,.feature-screen,.history-screen{position:relative;z-index:2}
.sidebar,.topbar,.input-glass,.quick-card,.glass-button,.search,.nav-item,.camera-box,.image-studio-card,.composer-menu,.task-view-panel,.utility-button,.region-clock,.network-toast,.task-card{
  background:linear-gradient(135deg,rgba(255,255,255,.58),rgba(224,248,255,.28))!important;
  border:1px solid rgba(255,255,255,.72)!important;
  box-shadow:inset 0 1px 0 rgba(255,255,255,.9),inset 0 -1px 0 rgba(56,145,182,.08),0 16px 48px rgba(26,112,148,.12)!important;
  backdrop-filter:blur(28px) saturate(155%)!important;-webkit-backdrop-filter:blur(28px) saturate(155%)!important;
}
.sidebar{width:285px!important;min-width:285px!important;padding:18px 16px!important;border-radius:0 28px 28px 0!important;overflow:hidden;transition:width .42s cubic-bezier(.2,.8,.2,1),min-width .42s cubic-bezier(.2,.8,.2,1),padding .42s ease,box-shadow .42s ease!important}
.logo{padding:5px 8px 8px!important}.logo-orb{background:linear-gradient(145deg,#c7f5ff,#4dc6f5)!important;box-shadow:0 0 32px rgba(58,190,239,.28)!important}
.new-chat.glass-button{min-height:46px!important;border-radius:16px!important;justify-content:flex-start!important;font-weight:650!important}
.search{height:43px!important;border-radius:15px!important}
.nav-item{border-radius:15px!important;transition:transform .24s ease,background .24s ease,box-shadow .24s ease!important}
.nav-item:hover{transform:translateX(2px)!important;background:rgba(255,255,255,.66)!important}.nav-item.active{background:rgba(255,255,255,.76)!important;box-shadow:0 9px 25px rgba(42,129,164,.12)!important}
.topbar{height:76px!important;margin:10px 12px 0;border-radius:24px!important;padding:10px 14px!important;gap:12px!important}
.topbar .status{flex:1!important;justify-content:center!important}
.status-dot{box-shadow:0 0 12px currentColor!important;animation:asritLiquidPulse 2.2s ease-in-out infinite}
.live-launch-button{display:inline-flex!important;align-items:center;gap:8px!important;border-radius:16px!important}
.live-dot-icon{width:8px;height:8px;border-radius:50%;background:#19c989;box-shadow:0 0 10px #19c989;display:inline-block}
.topbar-tools{display:flex;align-items:center;gap:8px}.utility-button{width:42px;height:42px;border-radius:14px;color:#0d2b3d!important;cursor:pointer;font-size:18px;display:grid;place-items:center;transition:.24s ease}.utility-button:hover{transform:translateY(-2px) scale(1.03)}
.wifi-symbol{font-size:22px;line-height:1;transform:rotate(90deg);display:block;transition:.3s ease}.network-button.offline .wifi-symbol{opacity:.42;transform:rotate(90deg) scale(.86)}
.region-clock{min-width:92px;height:42px;border-radius:14px;padding:4px 10px;display:grid;grid-template-columns:1fr;line-height:1.02}.region-clock strong{font-size:13px}.region-clock span{font-size:9px}.region-clock small{font-size:8px;color:#587286!important;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.chat{padding:24px 6vw 10px!important}.welcome{min-height:calc(100% - 10px)!important}.big-orb{background:linear-gradient(145deg,#eaffff,#65d2ff)!important;box-shadow:0 20px 80px rgba(28,166,215,.24)!important;animation:asritLiquidPulse 5s ease-in-out infinite}
.welcome h1{letter-spacing:-.055em!important}.quick-card{border-radius:22px!important;transition:transform .25s ease,box-shadow .25s ease!important}.quick-card:hover{transform:translateY(-4px) scale(1.01)!important;box-shadow:0 24px 55px rgba(22,117,153,.16)!important}
.input-area{padding:10px max(5vw,36px) 18px!important}.input-glass{border-radius:25px!important;min-height:68px!important;padding:9px!important}.input-glass .icon-button{background:rgba(255,255,255,.62)!important;border:1px solid rgba(255,255,255,.74)!important;transition:.2s ease!important}.input-glass .icon-button:hover{transform:translateY(-2px) scale(1.03)!important}.mic-mark{border-color:#0b3147!important}.mic-mark:before{background:#0b3147!important}.mic-mark:after{border-color:#0b3147!important}
/* Desktop collapsed sidebar: exactly the compact icon rail requested. */
.app.sidebar-collapsed .sidebar{width:78px!important;min-width:78px!important;padding:18px 10px!important;animation:asritSidebarOut .42s cubic-bezier(.2,.8,.2,1)}
.app.sidebar-collapsed .logo{justify-content:center;padding:4px 0 10px!important}.app.sidebar-collapsed .logo>div:last-child,.app.sidebar-collapsed .search,.app.sidebar-collapsed .history-title,.app.sidebar-collapsed .history{display:none!important}.app.sidebar-collapsed .new-chat{width:52px!important;min-width:52px!important;height:48px!important;padding:0!important;justify-content:center!important;margin:auto!important}.app.sidebar-collapsed .new-chat span:not(.new-chat-plus){display:none!important}.app.sidebar-collapsed .new-chat-plus{width:30px;height:30px}.app.sidebar-collapsed .side-nav{width:100%}.app.sidebar-collapsed .nav-item{width:58px!important;min-height:48px!important;padding:0!important;justify-content:center!important;margin:0 auto!important;font-size:0!important}.app.sidebar-collapsed .nav-icon{font-size:18px!important;width:auto!important}.app.sidebar-collapsed .nav-item:hover{transform:scale(1.04)!important}
/* Tooltip like the supplied compact-sidebar screenshot. */
.topbar .icon-button[title="Menu"]{position:relative}.topbar .icon-button[title="Menu"]:after{content:attr(data-tip);position:absolute;left:54px;top:50%;transform:translateY(-50%) translateX(-4px);opacity:0;pointer-events:none;background:#111;color:#fff!important;padding:7px 11px;border-radius:12px;white-space:nowrap;font:600 12px system-ui;box-shadow:0 10px 24px rgba(0,0,0,.2);transition:.18s ease;z-index:1000}.topbar .icon-button[title="Menu"]:hover:after{opacity:1;transform:translateY(-50%) translateX(0)}
/* Attachment launcher */
.composer-menu{position:fixed;left:calc(max(5vw,36px) + 4px);bottom:92px;width:min(380px,calc(100vw - 32px));border-radius:24px;padding:12px;display:none;z-index:80;animation:asritMenuIn .22s ease both}.composer-menu.open{display:block}.composer-menu-head{display:flex;justify-content:space-between;align-items:center;padding:10px 10px 12px}.composer-menu-head b{display:block;font-size:15px}.composer-menu-head small{display:block;color:#60798b!important;margin-top:3px}.composer-menu-head button{width:32px;height:32px;border:0;border-radius:10px;background:rgba(255,255,255,.5);font-size:20px;cursor:pointer}.composer-menu-item{width:100%;display:flex;gap:12px;align-items:center;text-align:left;padding:12px;border:0;border-radius:16px;background:transparent;cursor:pointer;transition:.2s ease}.composer-menu-item:hover{background:rgba(255,255,255,.56);transform:translateX(2px)}.composer-menu-item>span{width:38px;height:38px;border-radius:12px;display:grid;place-items:center;background:linear-gradient(145deg,#e7fbff,#85dafa);font-size:18px}.composer-menu-item b,.composer-menu-item small{display:block}.composer-menu-item small{margin-top:3px;color:#678092!important}
/* Task View */
.task-view-panel{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);width:min(720px,calc(100vw - 28px));border-radius:30px;padding:18px;z-index:120;display:none;animation:asritMenuIn .24s ease both}.task-view-panel.open{display:block}.task-view-head{display:flex;justify-content:space-between;align-items:center;padding:8px 8px 18px}.task-view-head span{font-size:10px;letter-spacing:.16em;color:#5b7a8c!important}.task-view-head h2{margin:4px 0 0;font-size:26px}.task-view-head button{width:38px;height:38px;border:0;border-radius:12px;background:rgba(255,255,255,.58);font-size:22px;cursor:pointer}.task-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.task-card{min-height:130px;border-radius:20px;padding:17px;text-align:left;cursor:pointer;transition:.22s ease}.task-card:hover{transform:translateY(-4px);box-shadow:0 22px 50px rgba(22,110,145,.16)!important}.task-card span{display:grid;place-items:center;width:38px;height:38px;border-radius:12px;background:linear-gradient(145deg,#e9fdff,#7bd5f7);margin-bottom:15px}.task-card b,.task-card small{display:block}.task-card small{margin-top:4px;color:#6a8191!important}
.network-toast{position:fixed;right:22px;bottom:22px;z-index:150;display:flex;align-items:center;gap:8px;padding:10px 14px;border-radius:16px;opacity:0;pointer-events:none}.network-toast.show{animation:asritToast 2.5s ease both}.network-icon{font-size:16px}.network-offline .network-icon{transform:rotate(90deg);opacity:.55}
/* Live modal: fluid open/close + glass controls */
.camera-panel{background:rgba(5,35,49,.22)!important;backdrop-filter:blur(16px)!important;transition:opacity .35s ease!important}.camera-box{border-radius:32px!important;overflow:hidden!important;box-shadow:0 30px 100px rgba(0,50,80,.28)!important;transform:translateY(10px) scale(.98);transition:transform .4s cubic-bezier(.2,.8,.2,1)!important}.camera-panel.open .camera-box{transform:none}.camera-controls .glass-button{border-radius:15px!important}.camera-controls .glass-button:hover{transform:translateY(-2px)!important}
/* Dark mode */
html.asrit-dark body{background:linear-gradient(160deg,#071b2b,#0a3850 48%,#062331)!important;color:#eafaff!important}html.asrit-dark body:before{opacity:.38}html.asrit-dark body:after{opacity:.28}html.asrit-dark .sidebar,html.asrit-dark .topbar,html.asrit-dark .input-glass,html.asrit-dark .quick-card,html.asrit-dark .glass-button,html.asrit-dark .search,html.asrit-dark .nav-item,html.asrit-dark .camera-box,html.asrit-dark .image-studio-card,html.asrit-dark .composer-menu,html.asrit-dark .task-view-panel,html.asrit-dark .utility-button,html.asrit-dark .region-clock,html.asrit-dark .task-card{background:linear-gradient(135deg,rgba(12,50,69,.72),rgba(7,30,43,.54))!important;border-color:rgba(170,235,255,.16)!important;color:#e9fbff!important}html.asrit-dark input,html.asrit-dark textarea,html.asrit-dark select{color:#e9fbff!important}html.asrit-dark #messageInput::placeholder{color:#9cb6c5!important}html.asrit-dark .nav-item,html.asrit-dark .status,html.asrit-dark .topbar *,html.asrit-dark .composer-menu-item *,html.asrit-dark .task-card *,html.asrit-dark .region-clock *{color:#e9fbff!important}html.asrit-dark .composer-menu-item small,html.asrit-dark .task-card small,html.asrit-dark .region-clock small{color:#9bb8c8!important}
@media(max-width:850px){.topbar-tools{gap:5px}.region-clock{display:none}.task-grid{grid-template-columns:repeat(2,1fr)}.sidebar{position:fixed!important;left:0;top:0;bottom:0;z-index:100}.app.sidebar-collapsed .sidebar{width:70px!important;min-width:70px!important}.topbar{margin-left:8px!important}.composer-menu{left:16px;bottom:86px}.chat{padding-left:18px!important;padding-right:18px!important}}
@media(max-width:560px){.topbar{height:64px!important}.utility-button{width:38px;height:38px}.task-grid{grid-template-columns:1fr}.live-launch-button{padding:10px!important}.input-glass{min-height:60px!important}.input-glass .icon-button{width:42px!important;height:42px!important;min-width:42px!important}.composer-menu{width:calc(100vw - 24px);left:12px}}
</style>

<script id="asrit-liquid-ocean-2026-js">
(function(){
  const app=document.querySelector('.app');
  const menuButton=document.querySelector('.topbar .icon-button[title="Menu"]');
  const fileInput=document.getElementById('fileInput');
  const cameraFileInput=document.getElementById('cameraFileInput');
  const composerMenu=document.getElementById('composerMenu');
  const taskPanel=document.getElementById('taskViewPanel');
  ...
  window.toggleSidebar=function(){
    ...
    localStorage.setItem('asrit-sidebar-collapsed',collapsed?'1':'0');
    if(menuButton){menuButton.dataset.tip=collapsed?'Open sidebar':'Close sidebar';menuButton.title=collapsed?'Open sidebar':'Close sidebar';}
  };
  const savedCollapsed=localStorage.getItem('asrit-sidebar-collapsed')==='1';
  if(savedCollapsed && app){app.classList.add('sidebar-collapsed');}
  if(menuButton){menuButton.dataset.tip=savedCollapsed?'Open sidebar':'Close sidebar';menuButton.title=savedCollapsed?'Open sidebar':'Close sidebar';}

  /* Dark mode */
  window.toggleDarkMode=function(){
    const on=root.classList.toggle('asrit-dark');
    localStorage.setItem('asrit-dark',on?'1':'0');
    const b=document.getElementById('themeButton');
    if(b){b.textContent=on?'☀':'◐';b.title=on?'Light mode':'Dark mode';}
  };
  if(localStorage.getItem('asrit-dark')==='1') root.classList.add('asrit-dark');
  const tb=document.getElementById('themeButton'); if(tb){tb.textContent=root.classList.contains('asrit-dark')?'☀':'◐';}

  /* Region-aware live clock. Uses the browser's configured timezone/locale. */
  function updateClock(){
    ...
    const shortZone=tz.split('/').pop().replaceAll('_',' ');
    const a=document.getElementById('regionTime'),b=document.getElementById('regionDate'),c=document.getElementById('regionName');
    if(a)a.textContent=time;if(b)b.textContent=date;if(c)c.textContent=shortZone;
  }
  updateClock();setInterval(updateClock,1000);

  /* Network status — browser real-time events */
  function updateNetwork(){
    const online=navigator.onLine;
    const status=document.getElementById('statusText');
    const dot=document.querySelector('.status-dot');
    const toast=document.getElementById('networkToast');
    const textEl=document.getElementById('networkToastText');
    document.body.classList.toggle('network-offline',!online);
    if(dot){dot.style.background=online?'#19c989':'#ff5f6d';dot.style.color=online?'#19c989':'#ff5f6d';}
    if(status && !status.dataset.busy) status.textContent=online?'ASRIT online':'No network connection';
    const nb=document.getElementById('networkButton');
    if(nb){nb.classList.toggle('offline',!online);nb.title=online?'Network connected':'Network disconnected';nb.setAttribute('aria-label',nb.title);}
    if(toast && textEl){textEl.textContent=online?'Network connected':'Network disconnected';toast.classList.remove('show');void toast.offsetWidth;toast.classList.add('show');}
  }
  window.addEventListener('online',updateNetwork);window.addEventListener('offline',updateNetwork);updateNetwork();

  /* Attachment popover */
  window.toggleComposerMenu=function(){
    if(!composerMenu)return;
    composerMenu.classList.toggle('open');
    composerMenu.setAttribute('aria-hidden',composerMenu.classList.contains('open')?'false':'true');
  };
  window.closeComposerMenu=function(){if(composerMenu)composerMenu.classList.remove('open');};
  window.chooseFiles=function(kind){
    closeComposerMenu();
    if(kind==='photo' && cameraFileInput){cameraFileInput.click();return;}
    if(fileInput)fileInput.click();
  };
  document.addEventListener('click',e=>{if(composerMenu && composerMenu.classList.contains('open') && !composerMenu.contains(e.target) && !e.target.closest('.add-button'))closeComposerMenu();});
  if(cameraFileInput){cameraFileInput.addEventListener('change',async()=>{const f=cameraFileInput.files?.[0];if(f){await window.uploadFile(f);}cameraFileInput.value='';});}

  /* Task View */
  window.openTaskView=function(){if(taskPanel){taskPanel.classList.add('open');taskPanel.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';}};
  window.closeTaskView=function(){if(taskPanel){taskPanel.classList.remove('open');taskPanel.setAttribute('aria-hidden','true');document.body.style.overflow='';}};
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeComposerMenu();closeTaskView();}});

  /* Upload: no blocking prompt; opening + should feel instant and support PC/mobile. */
  window.uploadFile=async function(file){
    if(!file)return;
    closeComposerMenu();
    if(typeof addMessage==='function') addMessage('user','📎 '+file.name);
    const status=document.getElementById('statusText');
    if(status){status.dataset.busy='1';status.textContent='ASRIT is reading '+file.name+'…';}
    const form=new FormData();form.append('file',file);form.append('message','Analyze this file and give me the most useful answer.');form.append('conversation_id',conversationId);
    try{
      const response=await fetch('/api/upload',{method:'POST',body:form});
      const data=await response.json();
      if(!response.ok)throw new Error(data.detail||data.error||'Upload failed');
      conversationId=data.conversation_id||conversationId;
      if(typeof addMessage==='function')addMessage('assistant',data.answer||'File received.');
      if(typeof speak==='function' && data.answer)speak(data.answer);
      if(typeof loadHistory==='function')loadHistory();
    }catch(err){if(typeof addMessage==='function')addMessage('assistant','File processing error: '+(err.message||err));}
    finally{if(status){status.dataset.busy='';status.textContent=navigator.onLine?'ASRIT online':'No network connection';}}
  };
  if(fileInput){fileInput.addEventListener('change',async()=>{for(const f of Array.from(fileInput.files||[]))await window.uploadFile(f);fileInput.value='';});}

  /* Voice button — clear visual state and restart safely after errors. */
  const oldToggleVoice=window.toggleVoice;
  window.toggleVoice=function(){
    if(typeof oldToggleVoice==='function')oldToggleVoice();
    setTimeout(()=>{
      const mic=document.getElementById('micButton');
      if(mic)mic.classList.toggle('recording',!!window.isRecording);
    },80);
  };

  /* Image Studio — current Nano Banana 2 endpoint, better errors, no dead button. */
  window.generateImage=async function(){
    const input=document.getElementById('imagePrompt'),result=document.getElementById('imageResult');
    const prompt=(input?.value||'').trim();if(!prompt||!result)return;
    result.innerHTML='<div class="image-generating"><span class="gen-orb"></span><b>ASRIT is creating…</b><small>Generating a high-quality image</small></div>';
    try{
      const form=new FormData();form.append('prompt',prompt);form.append('conversation_id',conversationId);
      const r=await fetch('/api/generate-image',{method:'POST',body:form});
      const data=await r.json();
      if(!r.ok||data.error)throw new Error(data.error||'Image generation failed');
      if(data.conversation_id)conversationId=data.conversation_id;
      const src=data.url+'?v='+Date.now();
      result.innerHTML='<img src="'+src+'" alt="ASRIT generated image" loading="eager"><div class="image-result-meta"><span>Generated by ASRIT</span><a href="'+src+'" target="_blank" rel="noopener">Open full image ↗</a></div>';
      if(typeof loadHistory==='function')loadHistory();
    }catch(err){result.innerHTML='<div class="image-error"><b>Image generation failed</b><small>'+String(err.message||err).replace(/[&<>]/g,'')+'</small></div>';}
  };

  /* Live stability: the Gemini 3.8 Live audio+video session has a short session
     lifetime, so rotate the session before expiry instead of leaving the UI on
     an endless "Listening…" state. */
  let liveRotationTimer=null;
  const originalOpenCamera=window.openCamera;
  window.openCamera=async function(){
    closeComposerMenu();
    if(typeof originalOpenCamera==='function')await originalOpenCamera();
    clearTimeout(liveRotationTimer);
    if(document.getElementById('cameraPanel')?.classList.contains('open')){
      liveRotationTimer=setTimeout(async()=>{
        if(document.getElementById('cameraPanel')?.classList.contains('open')){
          try{
            liveSetState('Refreshing secure Live session…',true);
            if(typeof window.closeCamera==='function')window.closeCamera();
            await new Promise(r=>setTimeout(r,450));
            if(typeof window.openCamera==='function')await window.openCamera();
          }catch(e){console.error(e)}
        }
      },105000);
    }
  };
  const originalCloseCamera=window.closeCamera;
  window.closeCamera=function(){clearTimeout(liveRotationTimer);liveRotationTimer=null;if(typeof originalCloseCamera==='function')originalCloseCamera();};

  /* Make Live mic/camera icons text-free and fluid. */
  const liveMic=document.getElementById('liveMicButton'),liveCam=document.getElementById('liveCameraButton');
  if(liveMic)liveMic.innerHTML='<span class="live-mic-icon" aria-hidden="true"></span>';
  if(liveCam)liveCam.innerHTML='<span class="live-camera-icon" aria-hidden="true"></span>';
})();
</script>

<style>
.image-generating{min-height:240px;display:grid;place-items:center;text-align:center;border-radius:24px;background:rgba(255,255,255,.38);border:1px solid rgba(255,255,255,.62);padding:30px}.image-generating b,.image-generating small{display:block}.image-generating small{margin-top:6px;color:#678092!important}.gen-orb{width:54px;height:54px;border-radius:50%;background:radial-gradient(circle at 30% 30%,#fff,#76d9ff 58%,#219fd1);box-shadow:0 0 45px rgba(38,179,229,.42);animation:asritLiquidPulse 1.5s ease-in-out infinite}.image-result-meta{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-top:10px;color:#567286!important}.image-result-meta a{color:#16739a!important;text-decoration:none}.image-error{padding:18px;border-radius:18px;background:rgba(255,94,94,.1);border:1px solid rgba(255,94,94,.18)}.image-error small{display:block;margin-top:5px}
.live-camera-icon{width:18px;height:13px;border:2px solid currentColor;border-radius:4px;display:block;position:relative}.live-camera-icon:after{content:"";position:absolute;right:-7px;top:2px;border-left:6px solid currentColor;border-top:4px solid transparent;border-bottom:4px solid transparent}.mic-button.recording{background:rgba(255,91,104,.18)!important;box-shadow:0 0 0 5px rgba(255,91,104,.08),0 0 24px rgba(255,91,104,.22)!important}.mic-button.recording .mic-mark{animation:asritLiquidPulse .7s ease-in-out infinite}
</style>
...
<style>
/* =========================================================
   ASRIT 2026 — SPATIAL DOCK REDESIGN
   Completely replaces the old sidebar composition.
   Navigation lives in a centered glass dock.
========================================================= */
...
:root{
  ...
  --a-blue:#38bdf2;
  --a-blue2:#79dcff;
  --a-glass:rgba(255,255,255,.48);
  --a-border:rgba(255,255,255,.72);
  --a-shadow:0 24px 80px rgba(18,99,135,.18);
  --a-dock:rgba(242,251,255,.62);
}

html,body{
  color:var(--a-ink)!important;
  background:
    radial-gradient(circle at 18% 18%,rgba(255,255,255,.95),transparent 32%),
    radial-gradient(circle at 84% 16%,rgba(91,219,255,.36),transparent 31%),
    linear-gradient(135deg,#f5fcff 0%,#c8f2ff 43%,#73d3f1 100%)!important;
}
...
body:before{
  ...
  left:-13vw;bottom:-23vw;
  border-radius:50%;
  background:radial-gradient(circle at 35% 35%,rgba(255,255,255,.74),rgba(80,203,244,.17) 45%,transparent 72%);
  filter:blur(5px);
  pointer-events:none;
  z-index:0;
  animation:asritFloatBlob 14s ease-in-out infinite alternate!important;
}
body:after{
  content:"";
  position:fixed;
  ...
  right:-13vw;top:8vh;
  border-radius:50%;
  background:radial-gradient(circle at 35% 30%,rgba(255,255,255,.58),rgba(28,177,225,.18) 48%,transparent 72%);
  filter:blur(8px);
  pointer-events:none;
  z-index:0;
  animation:asritFloatBlob2 17s ease-in-out infinite alternate!important;
}

@keyframes asritFloatBlob{from{transform:translate3d(0,0,0) scale(1)}to{transform:translate3d(7vw,-3vh,0) scale(1.08)}}
@keyframes asritFloatBlob2{from{transform:translate3d(0,0,0) scale(1)}to{transform:translate3d(-5vw,5vh,0) scale(1.12)}}
@keyframes asritDockIn{from{opacity:0;transform:translate(-50%,30px) scale(.92);filter:blur(8px)}to{opacity:1;transform:translate(-50%,0) scale(1);filter:blur(0)}}
@keyframes asritFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-9px)}}
@keyframes asritPulse{0%,100%{box-shadow:0 0 0 0 rgba(49,198,239,.22)}50%{box-shadow:0 0 0 10px rgba(49,198,239,0)}}
...
/* ---- remove the old composition completely ---- */
.sidebar{
  display:none!important;
}
.main{
...
.topbar{
  ...
  background:rgba(248,253,255,.48)!important;
  backdrop-filter:blur(28px) saturate(165%)!important;
  -webkit-backdrop-filter:blur(28px) saturate(165%)!important;
  box-shadow:0 20px 65px rgba(29,115,150,.14),inset 0 1px rgba(255,255,255,.92)!important;
...
.topbar:before{
  ...
  border-radius:15px;
  background:linear-gradient(145deg,#dff9ff,#54c9f2);
  color:#07334a;
  font:800 18px/1 system-ui,sans-serif;
  box-shadow:0 9px 24px rgba(30,161,208,.20),inset 0 1px rgba(255,255,255,.9);
}
.topbar:after{
  content:"ASRIT  •  PERSONAL AI";
  position:absolute;
  left:66px;
  top:50%;
...
.topbar .mobile-back{display:none!important}
.topbar .status{
...
.status-dot{
  box-shadow:0 0 0 5px rgba(42,205,139,.08),0 0 16px rgba(42,205,139,.34)!important;
}
...
.utility-button:hover{background:rgba(255,255,255,.66)!important;transform:translateY(-2px)!important}
...
.region-clock strong{font-size:13px!important}
.region-clock span{font-size:9px!important}
.region-clock small{font-size:8px!important;color:#547486!important}

.live-launch-button{
  min-width:78px!important;
  height:42px!important;
  border-radius:15px!important;
  background:linear-gradient(145deg,rgba(255,255,255,.72),rgba(222,249,255,.42))!important;
  color:#0b3044!important;
  border:1px solid rgba(255,255,255,.84)!important;
  box-shadow:inset 0 1px rgba(255,255,255,.9),0 8px 24px rgba(20,128,169,.10)!important;
}
.live-launch-button:hover{transform:translateY(-2px)!important}
.live-dot-icon{
  box-shadow:0 0 0 4px rgba(36,210,145,.09),0 0 13px rgba(36,210,145,.45)!important;
}
...
.chat{
  ...
  padding:118px 24px 178px!important;
  overflow:auto!important;
  background:transparent!important;
  scrollbar-width:thin;
  scrollbar-color:rgba(21,115,151,.22) transparent;
}
...
.chat:before{
  ...
  top:19%;left:8%;
  border-radius:50%;
  background:radial-gradient(circle at 35% 30%,rgba(255,255,255,.62),rgba(50,201,242,.12) 52%,transparent 72%);
  filter:blur(2px);
  pointer-events:none;
  animation:asritFloat 8s ease-in-out infinite;
}
...
.big-orb{
  ...
  background:
    radial-gradient(circle at 30% 23%,#ffffff 0 9%,rgba(255,255,255,.70) 10%,transparent 26%),
    linear-gradient(145deg,#bff3ff,#43c7f2 62%,#149ed2)!important;
  color:#06344a!important;
  font-size:43px!important;
  font-weight:850!important;
  box-shadow:0 25px 70px rgba(18,151,201,.28),inset 0 2px 2px rgba(255,255,255,.92)!important;
  animation:asritFloat 5.5s ease-in-out infinite!important;
}
.welcome h1{
  margin:0!important;
...
/* ---- quick actions become a clean floating command matrix ---- */
.quick-actions{
  display:grid!important;
...
.quick-card{
  ...
  background:rgba(247,253,255,.40)!important;
  backdrop-filter:blur(20px)!important;
  -webkit-backdrop-filter:blur(20px)!important;
  box-shadow:0 14px 42px rgba(26,120,151,.09),inset 0 1px rgba(255,255,255,.84)!important;
  color:#102e41!important;
}
.quick-card:hover{
  transform:translateY(-5px)!important;
  background:rgba(255,255,255,.64)!important;
  box-shadow:0 24px 55px rgba(24,130,164,.15),inset 0 1px rgba(255,255,255,.95)!important;
}
.quick-card strong{display:block!important;color:#0a2d41!important;font-size:13px!important}
.quick-card small{display:block!important;color:#557283!important;margin-top:7px!important;line-height:1.35!important}
...
.message-bubble{
  ...
  box-shadow:0 16px 42px rgba(20,102,133,.10),inset 0 1px rgba(255,255,255,.86)!important;
}
.message.user .message-bubble{
  background:linear-gradient(135deg,rgba(224,248,255,.82),rgba(255,255,255,.56))!important;
  color:#08283b!important;
}
.response-action{color:#315466!important}
.response-action:hover{background:rgba(255,255,255,.55)!important;color:#08283b!important}

/* ---- composer is an independent floating control ---- */
.input-area{
  position:fixed!important;
...
.input-glass{
  ...
  align-items:center!important;
  gap:5px!important;
  background:rgba(248,254,255,.58)!important;
  border:1px solid rgba(255,255,255,.84)!important;
  backdrop-filter:blur(30px) saturate(155%)!important;
  -webkit-backdrop-filter:blur(30px) saturate(155%)!important;
  box-shadow:0 22px 70px rgba(18,111,145,.18),inset 0 1px rgba(255,255,255,.95)!important;
}
.input-glass:focus-within{
  box-shadow:0 24px 80px rgba(18,111,145,.22),0 0 0 4px rgba(70,203,243,.10),inset 0 1px rgba(255,255,255,.95)!important;
}
.input-glass #messageInput{
  background:transparent!important;
  color:#092a3e!important;
  min-height:48px!important;
  border:0!important;
  box-shadow:none!important;
}
#messageInput::placeholder{color:#567487!important}
.input-glass .icon-button{
...
.input-glass .icon-button:hover{background:rgba(255,255,255,.80)!important;transform:translateY(-2px)!important}
.input-glass .send-button{
  background:linear-gradient(145deg,#9ce9ff,#4cc3ee)!important;
  border-color:rgba(255,255,255,.84)!important;
}

/* ---- new centered macOS-like dock ---- */
.mobile-dock{
  position:fixed!important;
  ...
  padding:7px!important;
  border-radius:24px!important;
  background:rgba(246,253,255,.58)!important;
  border:1px solid rgba(255,255,255,.82)!important;
  backdrop-filter:blur(32px) saturate(170%)!important;
  -webkit-backdrop-filter:blur(32px) saturate(170%)!important;
  box-shadow:0 20px 70px rgba(16,100,133,.20),inset 0 1px rgba(255,255,255,.94)!important;
  animation:asritDockIn .48s cubic-bezier(.2,1.15,.3,1) both!important;
}
.dock-btn{
  position:relative!important;
  display:grid!important;
...
.dock-btn:hover span{
  background:linear-gradient(145deg,#dff9ff,#72d7f7)!important;
  box-shadow:0 12px 25px rgba(23,157,205,.20),inset 0 1px rgba(255,255,255,.95)!important;
}
.dock-btn:active{transform:translateY(-2px) scale(.95)!important}
.dock-btn.is-primary span{
  background:linear-gradient(145deg,#b9f3ff,#4bc5ef)!important;
  box-shadow:0 10px 28px rgba(28,169,217,.23),inset 0 1px rgba(255,255,255,.95)!important;
}
.dock-btn[title]:after{
  ...
  padding:7px 10px;
  border-radius:10px;
  background:rgba(8,34,49,.92);
  color:#fff!important;
  font:600 11px/1 system-ui,sans-serif;
  transition:.18s ease;
}
.dock-btn:hover[title]:after{opacity:1;transform:translate(-50%,0)}
...
.composer-menu{
  ...
  border-radius:24px!important;
  background:rgba(248,254,255,.72)!important;
  border:1px solid rgba(255,255,255,.86)!important;
  backdrop-filter:blur(30px)!important;
  -webkit-backdrop-filter:blur(30px)!important;
  box-shadow:0 28px 80px rgba(14,91,124,.20)!important;
}

/* ---- image studio becomes a floating sheet ---- */
.image-studio{
  position:absolute!important;
  inset:0!important;
...
.image-studio-card{
  ...
  background:rgba(248,254,255,.56)!important;
  border:1px solid rgba(255,255,255,.84)!important;
  backdrop-filter:blur(30px)!important;
  -webkit-backdrop-filter:blur(30px)!important;
  box-shadow:0 28px 90px rgba(19,107,140,.18),inset 0 1px rgba(255,255,255,.95)!important;
  color:#0b293b!important;
}
.image-studio-card h2,.image-studio-card p,.image-studio-card div{color:inherit!important}
.image-studio-card p{color:#476a7c!important}
.image-prompt{
  color:#092a3e!important;
  background:rgba(255,255,255,.48)!important;
  border:1px solid rgba(255,255,255,.84)!important;
...
/* ---- make every option/icon explicit and visible ---- */
.nav-icon,.plus-mark,.mic-mark,.send-mark,
.dock-btn span,.utility-button{
  opacity:1!important;
...
/* =========================================================
   DARK MODE — white text everywhere, no muddy black text
========================================================= */
html.asrit-dark,html.asrit-dark body{
  color:#f4fcff!important;
  background:
    radial-gradient(circle at 20% 16%,rgba(62,185,230,.18),transparent 30%),
    radial-gradient(circle at 80% 20%,rgba(28,132,174,.18),transparent 30%),
    linear-gradient(145deg,#061724 0%,#082c40 46%,#04141f 100%)!important;
}
html.asrit-dark .topbar,
html.asrit-dark .mobile-dock,
html.asrit-dark .input-glass,
html.asrit-dark .quick-card,
html.asrit-dark .message-bubble,
html.asrit-dark .image-studio-card,
html.asrit-dark .composer-menu,
html.asrit-dark .utility-button,
html.asrit-dark .region-clock,
html.asrit-dark .task-view-panel,
html.asrit-dark .task-card{
  background:linear-gradient(145deg,rgba(12,48,66,.68),rgba(3,25,36,.58))!important;
  border-color:rgba(177,237,255,.18)!important;
  color:#f4fcff!important;
  box-shadow:0 24px 80px rgba(0,0,0,.28),inset 0 1px rgba(255,255,255,.10)!important;
}
html.asrit-dark .topbar:after{color:#f3fcff!important}
html.asrit-dark .topbar:before{color:#06354a!important}
html.asrit-dark .topbar .status,
html.asrit-dark .utility-button,
html.asrit-dark .region-clock{
  color:#f4fcff!important;
}
html.asrit-dark .region-clock small{color:#b5cfda!important}
html.asrit-dark .welcome h1,
html.asrit-dark .welcome p,
html.asrit-dark .quick-card strong,
html.asrit-dark .quick-card small,
html.asrit-dark .message-bubble,
html.asrit-dark .response-action,
html.asrit-dark .image-studio-card,
html.asrit-dark .image-studio-card h2,
html.asrit-dark .image-studio-card p,
html.asrit-dark .dock-btn,
html.asrit-dark .input-glass #messageInput,
html.asrit-dark .image-prompt,
html.asrit-dark .task-card,
html.asrit-dark .task-card *,
html.asrit-dark .composer-menu-item *,
html.asrit-dark .live-state,
html.asrit-dark .live-meta,
html.asrit-dark .camera-controls{
  color:#f4fcff!important;
}
html.asrit-dark #messageInput::placeholder,
html.asrit-dark .image-prompt::placeholder{color:#a9c5d1!important}
html.asrit-dark .quick-card small,
html.asrit-dark .image-studio-card p{color:#b6cdd7!important}
html.asrit-dark .dock-btn span{
  color:#f2fcff!important;
  background:rgba(255,255,255,.10)!important;
  border-color:rgba(180,237,255,.16)!important;
}
html.asrit-dark .dock-btn:hover{background:rgba(255,255,255,.10)!important}
html.asrit-dark .dock-btn:hover span,
html.asrit-dark .dock-btn.is-primary span{
  color:#073349!important;
  background:linear-gradient(145deg,#baf2ff,#4bbfe7)!important;
}
html.asrit-dark .input-glass .icon-button{
  color:#f5fdff!important;
  background:rgba(255,255,255,.10)!important;
  border-color:rgba(180,237,255,.18)!important;
}
html.asrit-dark .send-mark,
html.asrit-dark .plus-mark{color:#f5fdff!important}
html.asrit-dark .mic-mark{border-color:#f5fdff!important}
html.asrit-dark .mic-mark:after{border-color:#f5fdff!important;border-top:0!important}
html.asrit-dark .mic-mark:before{background:#f5fdff!important}
html.asrit-dark .message.user .message-bubble{
  background:linear-gradient(145deg,rgba(22,94,125,.64),rgba(9,51,72,.58))!important;
  color:#fff!important;
}
html.asrit-dark .image-prompt{
  background:rgba(4,25,36,.52)!important;
  border-color:rgba(180,237,255,.18)!important;
}
html.asrit-dark .composer-menu-item:hover{background:rgba(255,255,255,.08)!important}
html.asrit-dark .composer-menu-item>span{
  background:linear-gradient(145deg,#184f68,#0d789d)!important;
  color:#fff!important;
}
html.asrit-dark .task-card small{color:#b2cbd6!important}

/* Final readability pass: light mode uses deep ink; dark mode uses white text. */
html:not(.asrit-dark) .main,
html:not(.asrit-dark) .main button,
html:not(.asrit-dark) .main input,
html:not(.asrit-dark) .main textarea,
html:not(.asrit-dark) .main select{color:#102b3d!important}
html.asrit-dark .main,
html.asrit-dark .main button,
html.asrit-dark .main input,
html.asrit-dark .main textarea,
html.asrit-dark .main select,
html.asrit-dark .main label,
html.asrit-dark .main p,
html.asrit-dark .main h1,
html.asrit-dark .main h2,
html.asrit-dark .main h3,
html.asrit-dark .main h4,
html.asrit-dark .main h5,
html.asrit-dark .main h6,
html.asrit-dark .main span,
html.asrit-dark .main small,
html.asrit-dark .main strong,
html.asrit-dark .main div{color:#f4fcff!important}
html.asrit-dark .topbar:before{
  color:#ffffff!important;
  background:linear-gradient(145deg,#0c5f7c,#0c9ac8)!important;
}
html.asrit-dark .status-dot{background:#31d98e!important}
html.asrit-dark .live-dot-icon{background:#31d98e!important}

/* ---- responsive dock ---- */
@media(max-width:900px){
  .quick-actions{grid-template-columns:repeat(2,1fr)!important}
  .topbar{width:calc(100vw - 24px)!important}
  .topbar:after{content:"ASRIT";left:66px}
  .region-clock{display:none!important}
}
@media(max-width:650px){
  .topbar{top:10px!important;height:58px!important;border-radius:20px!important}
  .topbar:before{width:38px;height:38px;flex-basis:38px;border-radius:13px}
  .topbar:after{left:58px}
  .topbar .status{padding:8px 9px!important;font-size:10px!important}
  .utility-button{width:36px!important;height:36px!important}
  .live-launch-button{height:36px!important;min-width:66px!important}
  .chat{padding:88px 14px 176px!important}
  .welcome{min-height:calc(100vh - 270px)!important}
  .big-orb{width:84px!important;height:84px!important;border-radius:27px!important;font-size:34px!important}
  ...
  .input-area{width:calc(100vw - 20px)!important;bottom:82px!important}
  .input-glass{border-radius:20px!important}
  .mobile-dock{
  ...
  .dock-btn{width:48px!important;min-width:48px!important;height:50px!important}
  .dock-btn span{width:29px!important;height:29px!important;font-size:14px!important}
}
</style>
...
(function(){
  ...
  /* Replace the old four-button mobile rail with the new spatial dock.
     All actions call the app's existing functions, so backend features stay intact. */
  dock.innerHTML = `
    <button class="dock-btn is-primary" title="New conversation" onclick="newChat()">
      <span aria-hidden="true">＋</span>
    </button>
    <button class="dock-btn" title="Chat" onclick="showMode('chat')">
      <span aria-hidden="true">◌</span>
    </button>
    <button class="dock-btn" title="Create images" onclick="showMode('images')">
      <span aria-hidden="true">✦</span>
    </button>
    <button class="dock-btn" title="ASRIT Live" onclick="openCamera()">
      <span aria-hidden="true">◉</span>
    </button>
    <button class="dock-btn" title="History" onclick="openHistoryScreen()">
      <span aria-hidden="true">◷</span>
    </button>
    <button class="dock-btn" title="Library" onclick="openFeature('library')">
      <span aria-hidden="true">▤</span>
    </button>
    <button class="dock-btn" title="Scheduled" onclick="openFeature('scheduled')">
      <span aria-hidden="true">◫</span>
    </button>
    <button class="dock-btn" title="Plugins" onclick="openFeature('plugins')">
      <span aria-hidden="true">⌘</span>
    </button>
    <button class="dock-btn" title="Projects" onclick="openFeature('projects')">
      <span aria-hidden="true">□</span>
    </button>
    <button class="dock-btn" title="Codex" onclick="openFeature('codex')">
      <span aria-hidden="true">{ }</span>
    </button>
  `;

  /* Prevent old mobile-dock rules/scripts from hiding the new desktop dock. */
  dock.setAttribute('aria-label','ASRIT navigation dock');

  /* Keyboard shortcut: Cmd/Ctrl + K opens the add menu. */
  document.addEventListener('keydown',function(e){
    if((e.ctrlKey||e.metaKey) && e.key.toLowerCase()==='k'){
      e.preventDefault();
      if(typeof toggleComposerMenu==='function') toggleComposerMenu();
    }
  });

  /* Add a tiny active-state cue without replacing existing mode logic. */
  document.addEventListener('click',function(e){
    const btn=e.target.closest('.dock-btn');
    if(!btn) return;
    dock.querySelectorAll('.dock-btn').forEach(x=>x.classList.remove('is-active'));
...
<!-- =========================================================
     ASRIT 2026 — CLEAN FLOATING WORKSPACE PATCH
     Keeps existing functionality; fixes spacing, dock labels,
     composer visibility, automatic response positioning and Live UI.
========================================================= -->
<style>
/* The dock is a navigation shelf, not a second composer. */
.mobile-dock{
  position:fixed!important;
  left:50%!important;
  ...
  border-radius:24px!important;
  overflow:visible!important;
  background:rgba(247,253,255,.76)!important;
  border:1px solid rgba(255,255,255,.90)!important;
  backdrop-filter:blur(26px) saturate(145%)!important;
  -webkit-backdrop-filter:blur(26px) saturate(145%)!important;
  box-shadow:0 16px 45px rgba(13,83,111,.16),inset 0 1px rgba(255,255,255,.95)!important;
}
...
.mobile-dock .dock-icon{
  ...
  background:rgba(255,255,255,.58)!important;
  border:1px solid rgba(116,185,210,.18)!important;
...
.mobile-dock .dock-btn:hover{
  background:rgba(105,205,236,.14)!important;
  transform:translateY(-3px)!important;
}
.mobile-dock .dock-btn:hover .dock-icon{
  background:linear-gradient(145deg,#e8fbff,#9be5f9)!important;
  box-shadow:0 7px 18px rgba(32,153,194,.16),inset 0 1px rgba(255,255,255,.98)!important;
}
.mobile-dock .dock-btn.is-primary .dock-icon,
.mobile-dock .dock-btn.is-active .dock-icon{
  background:linear-gradient(145deg,#baf3ff,#55c8ec)!important;
  color:#07364c!important;
  box-shadow:0 7px 20px rgba(29,161,205,.22),inset 0 1px rgba(255,255,255,.98)!important;
}
.mobile-dock .dock-btn.is-primary .dock-label,
.mobile-dock .dock-btn.is-active .dock-label{color:#07364c!important}

/* The composer gets its own lane above the dock. */
.input-area{
  position:fixed!important;
  left:50%!important;
...
.input-glass{
  ...
  gap:7px!important;
  border-radius:22px!important;
  background:rgba(249,254,255,.78)!important;
  border:1px solid rgba(255,255,255,.92)!important;
  backdrop-filter:blur(25px) saturate(150%)!important;
  -webkit-backdrop-filter:blur(25px) saturate(150%)!important;
  box-shadow:0 14px 42px rgba(13,83,111,.15),inset 0 1px rgba(255,255,255,.98)!important;
}
.input-glass #messageInput{color:#102f42!important}
#messageInput::placeholder{color:#5e7788!important}
.input-glass .icon-button,
.input-glass .add-button,
.input-glass #micButton,
.input-glass .send-button{
...
.input-glass #micButton{
  background:rgba(233,248,253,.96)!important;
  color:#0b405a!important;
  border:1px solid rgba(52,133,160,.20)!important;
}
.input-glass #micButton .mic-mark{
  display:block!important;visibility:visible!important;opacity:1!important;
  width:12px!important;height:17px!important;
  border:2px solid #0b405a!important;
  border-radius:8px!important;
  position:relative!important;
}
.input-glass #micButton .mic-mark:after{
  content:""!important;position:absolute!important;left:50%!important;bottom:-7px!important;
  width:15px!important;height:8px!important;transform:translateX(-50%)!important;
  border:2px solid #0b405a!important;border-top:0!important;border-radius:0 0 9px 9px!important;
}
.input-glass #micButton .mic-mark:before{
  content:""!important;position:absolute!important;left:50%!important;bottom:-10px!important;
  width:2px!important;height:5px!important;transform:translateX(-50%)!important;
  background:#0b405a!important;border-radius:2px!important;
}
.input-glass #micButton.recording{background:rgba(255,100,112,.18)!important}

/* Chat always reserves space for both floating controls. */
.chat{
...
.message.assistant,.message.user{scroll-margin-top:90px!important;scroll-margin-bottom:225px!important}

/* Never let response controls disappear underneath the composer. */
.response-actions{position:relative!important;z-index:2!important}
.message.assistant .message-bubble{max-width:min(760px,calc(100% - 20px))!important}

/* Live panel stays above the dock and has a clean control row. */
#cameraPanel{z-index:500!important}
#cameraPanel .camera-box{margin-bottom:120px!important}
#liveMicButton,#liveCameraButton,#liveFlipCameraButton{
  min-width:48px!important;min-height:48px!important;
  display:grid!important;place-items:center!important;
  visibility:visible!important;opacity:1!important;
}

/* Dark mode: labels, controls and composer are all readable. */
html.asrit-dark .mobile-dock{
  background:rgba(5,27,39,.80)!important;
  border-color:rgba(170,235,255,.18)!important;
  box-shadow:0 18px 55px rgba(0,0,0,.32),inset 0 1px rgba(255,255,255,.10)!important;
}
html.asrit-dark .mobile-dock .dock-btn{color:#f4fcff!important}
html.asrit-dark .mobile-dock .dock-icon{
  color:#effcff!important;background:rgba(255,255,255,.09)!important;border-color:rgba(180,237,255,.15)!important;
}
html.asrit-dark .mobile-dock .dock-label{color:#eafaff!important}
html.asrit-dark .mobile-dock .dock-btn:hover{background:rgba(255,255,255,.08)!important}
html.asrit-dark .mobile-dock .dock-btn.is-primary .dock-icon,
html.asrit-dark .mobile-dock .dock-btn.is-active .dock-icon{color:#07364c!important}
html.asrit-dark .input-glass{
  background:rgba(7,34,48,.84)!important;border-color:rgba(170,235,255,.18)!important;
}
html.asrit-dark .input-glass #messageInput{color:#fff!important}
html.asrit-dark #messageInput::placeholder{color:#a9c5d1!important}
html.asrit-dark .input-glass #micButton{background:rgba(255,255,255,.10)!important;color:#fff!important}
html.asrit-dark .input-glass #micButton .mic-mark{border-color:#fff!important}
html.asrit-dark .input-glass #micButton .mic-mark:after{border-color:#fff!important;border-top:0!important}
html.asrit-dark .input-glass #micButton .mic-mark:before{background:#fff!important}

@media(max-width:1100px){
  .mobile-dock .dock-btn{width:70px!important;min-width:70px!important}
}
@media(max-width:760px){
  .mobile-dock{
    left:10px!important;right:10px!important;transform:none!important;
    max-width:none!important;width:auto!important;overflow-x:auto!important;
    justify-content:flex-start!important;scrollbar-width:none!important;
  }
  .mobile-dock::-webkit-scrollbar{display:none}
  .mobile-dock .dock-btn{width:68px!important;min-width:68px!important}
  .input-area{bottom:100px!important;width:calc(100vw - 20px)!important}
  .chat{padding-bottom:205px!important}
}
</style>
<script>
(function(){
  ...
  /* Give every destination a visible name, while keeping the dock compact. */
  dock.innerHTML = `
    <button class="dock-btn is-primary" title="New conversation" onclick="newChat()"><span class="dock-icon">＋</span><small class="dock-label">New</small></button>
    <button class="dock-btn" title="Chat" onclick="showMode('chat')"><span class="dock-icon">◌</span><small class="dock-label">Chat</small></button>
    <button class="dock-btn" title="Images" onclick="showMode('images')"><span class="dock-icon">✦</span><small class="dock-label">Images</small></button>
    <button class="dock-btn" title="ASRIT Live" onclick="openCamera()"><span class="dock-icon">◉</span><small class="dock-label">Live</small></button>
    <button class="dock-btn" title="History" onclick="openHistoryScreen()"><span class="dock-icon">◷</span><small class="dock-label">History</small></button>
    <button class="dock-btn" title="Library" onclick="openFeature('library')"><span class="dock-icon">▤</span><small class="dock-label">Library</small></button>
    <button class="dock-btn" title="Scheduled" onclick="openFeature('scheduled')"><span class="dock-icon">◫</span><small class="dock-label">Schedule</small></button>
    <button class="dock-btn" title="Plugins" onclick="openFeature('plugins')"><span class="dock-icon">⌘</span><small class="dock-label">Plugins</small></button>
    <button class="dock-btn" title="Projects" onclick="openFeature('projects')"><span class="dock-icon">□</span><small class="dock-label">Projects</small></button>
    <button class="dock-btn" title="Codex" onclick="openFeature('codex')"><span class="dock-icon">{ }</span><small class="dock-label">Codex</small></button>
  `;

  /* Keep the newest AI reply above the floating controls. */
  ...
  /* Make the thinking state visible immediately and keep it out of the dock lane. */
  const originalSendMessage=window.sendMessage;
  if(typeof originalSendMessage==='function'){
    window.sendMessage=async function(){
      const chat=document.getElementById('chat');
      if(chat) chat.classList.add('asrit-requesting');
      try{return await originalSendMessage();}
      finally{if(chat) chat.classList.remove('asrit-requesting');}
    };
  }
})();
</script>
